...
writeFileSync(join(generatedDir, 'summary.json'), JSON.stringify({
  ...
  contexts: contexts.map((ctx) => ({
    ...
    counts: ctx.counts,
    modules: ctx.modules.map((m) => ({ id: m.id, title: m.title, kind: m.kind, fileCount: m.files.length, ownedConcerns: m.ownedConcerns })),
  })),
}, null, 2) + '\n');
writeFileSync(join(generatedDir, 'source-first.c4'), render(contexts));

console.log(`Generated source-first architecture v2 at ${baselineSha}`);
for (const ctx of contexts) {
...
function loadContext(def) {
  ...
  const sourceFiles = files.filter((f) => f.relativePath.startsWith('src/') && /\.(?:[cm]?js|jsx|ts|tsx)$/.test(f.relativePath)).map((file) => {
  ...
  const modules = [...groups.entries()].map(([key, moduleFiles]) => buildModule(key, moduleFiles, def.path)).sort((a, b) => a.title.localeCompare(b.title));
  ...
  return { id: def.id, path: def.path, packageName, packageConcern, files, sourceFiles, modules, internalEdges: [...internal.values()], workspaceEdges: [], counts };
}
...
function deriveWorkspaceEdges(ctx, packageByName) {
  ...
  ctx.workspaceEdges = [...counts].map(([to, count]) => ({ from: ctx.id, to, count })).sort((a, b) => a.to.localeCompare(b.to));
}
...
function parseTreeLine(line, scope) {
  ...
  return { mode, type, blobSha, sizeBytes: rawSize === '-' ? null : Number(rawSize), path, relativePath,
...
function first(content, regexes) { for (const regex of regexes) { const value = content.match(regex)?.[1]?.trim(); if (value) return value; } }
function headerSummary(content) {
  const result = [];
  for (const line of content.split(/\r?\n/).slice(0, 35)) {
    if (/^\s*(?:export|import|const|let|class|interface|type|function)\b/.test(line)) break;
    const text = line.replace(/^\s*\/\*\*?\s?/, '').replace(/^\s*\*\s?/, '').replace(/\*\/\s*$/, '').replace(/^\s*\/\/\s?/, '').trim();
    if (!text || /^[-─=]+$/.test(text) || text.startsWith('@') || /^Governing:/i.test(text)) continue;
    result.push(text);
  }
  return result.slice(0, 2).join(' ').trim() || undefined;
}
function parseImports(content) {
  const values = new Set();
  for (const m of content.matchAll(/(?:from\s+|import\s*\(\s*|require\s*\(\s*)['"]([^'"]+)['"]/g)) values.add(m[1]);
  for (const m of content.matchAll(/(?:^|\n)\s*import\s+['"]([^'"]+)['"]/g)) values.add(m[1]);
  return [...values];
}
...
function humanize(value) { return value.replace(/([a-z0-9])([A-Z])/g, '$1 $2').replace(/[-_]+/g, ' ').replace(/\b\w/g, (m) => m.toUpperCase()); }
...
function inventoryPayload(ctx) {
  return {
    ...
    packageConcern: ctx.packageConcern,
    counts: ctx.counts,
    modules: ctx.modules.map((m) => ({ id: m.id, key: m.key, title: m.title, kind: m.kind, provenance: 'STRUCTURE-DERIVED', sourcePath: m.sourcePath, fileCount: m.files.length, ownedConcerns: m.ownedConcerns, decisions: m.decisions, files: m.files.map((f) => f.relativePath) })),
...
function render(all) {
  const lines = ['// GENERATED FILE. DO NOT EDIT.', `// Source baseline: ${config.repository}@${baselineSha}`, 'model {', "  dvt = system 'DVT+ — source-first architecture' {", '    #asIs', "    description 'Generated from the current Git tree. Package/module structure and dependency arrows are source-derived; target architecture is never promoted automatically.'", '    metadata {', `      repository '${esc(config.repository)}'`, `      baselineSha '${baselineSha}'`, `      selectedContexts '${all.length}'`, '    }'];
  ...
  for (const ctx of all) {
    const inv = `inventory_${safeId(ctx.id)}`;
    const refs = new Map();
    evidenceRefs.set(ctx.id, refs);
    lines.push(`  ${inv} = inventory '${esc(ctx.packageName)} — exact Git evidence' {`);
    ...
    for (const file of ctx.files) {
      ...
      refs.set(file.relativePath, `${inv}.${id}`);
  ...
  for (const ctx of all) for (const edge of ctx.workspaceEdges) lines.push(`  dvt.pkg_${safeId(edge.from)} .uses dvt.pkg_${safeId(edge.to)} 'workspace imports (${edge.count})'`);
  lines.push('}', '', 'views {');
  ...
  for (const ctx of all) {
    ...
    const inv = `inventory_${safeId(ctx.id)}`;
    lines.push(`  view ${safeId(ctx.id)}Inventory of ${inv} {`);
    ...
    for (const [suffix, predicate, title] of [
      ['Tests', (p) => /^tests?\//.test(p), 'tests'],
      ['Docs', (p) => p.startsWith('docs/'), 'docs'],
      ['Root', (p) => !p.includes('/'), 'package root files'],
    ]) {
