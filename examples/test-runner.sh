...
validate_golden_path() {
  ...
  schema_version=$(python3 -c "import json; print(json.load(open('$path_dir/plan.v1.1.json'))['schemaVersion'])" 2>/dev/null || echo "")
  ...
  validation_status=$(python3 -c "import json; print(json.load(open('$path_dir/validation-report.json'))['status'])" 2>/dev/null || echo "")
