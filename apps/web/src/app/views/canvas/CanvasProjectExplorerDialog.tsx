...
export function CanvasProjectExplorerDialog({
  ...
  return (
    ...
    >
      <section className="flex max-h-full w-full max-w-4xl flex-col overflow-hidden rounded-md border border-[color:var(--border-default)] bg-[var(--surface-panel)] shadow-2xl">
        <header className="border-b border-[color:var(--border-muted)] px-5 py-4">
          ...
          <label className="mt-4 block text-xs font-semibold uppercase tracking-wide text-(--text-muted)">
            ...
            <input
              ...
              className="mt-2 w-full rounded border border-[color:var(--border-default)] bg-[var(--surface-panel-subtle)] px-3 py-2 text-sm font-normal normal-case tracking-normal text-(--text-default) outline-none focus:border-[color:var(--accent-default)]"
        ...
        <div className="min-h-0 overflow-y-auto p-4">
          {filteredDocuments.length === 0 ? (
            <p className="rounded border border-[color:var(--border-muted)] px-4 py-6 text-sm text-(--text-muted)">
              ...
              {filteredDocuments.map((canvasDocument) => {
                ...
                return (
                  <li
                    ...
                    className="rounded border border-[color:var(--border-default)] bg-[var(--surface-panel-subtle)] p-3"
                  >
                    <div className="flex flex-wrap items-center justify-between gap-3">
                      ...
                      {selected ? (
                        <span className="rounded border border-[color:var(--status-info)] px-2 py-1 text-xs text-(--status-info)">
                          Current
                        </span>
                      ) : (
                        <button
