...
describe('canvas authoring runtime component architecture', () => {
  it('keeps the runtime contract, draft-flow seam, baseline seam, and lifecycle seam explicitly separated', () => {
    ...
    expect(RUNTIME_CONTRACT_SOURCE).toContain('export type UseCanvasAuthoringRuntimeDraftFlowArgs');
    ...
    expect(RUNTIME_SOURCE).toContain('canPersistGraphDraftTransport &&');
    expect(RUNTIME_SOURCE).toContain('canPersistDraftTransport && canMutateGraphTransport');
    expect(RUNTIME_SOURCE).not.toContain('canEditDraftTransport &&');
    ...
    expect(BASELINE_SOURCE).toContain(
      'Owned concern: provide the Canvas authoring-runtime baseline'
    );
