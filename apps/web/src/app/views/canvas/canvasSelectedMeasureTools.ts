...
export function aggregateTool(input: SelectedRelationInput): CanvasRelationalOperatorTool {
  ...
  const ordinal = readDvtSubstraitFieldReferenceOrdinal(aggregate?.groupingExpressions[0]);
  return {
    id: 'aggregate',
    ...
    enabled:
      ...
      (input.intent === 'insert' ||
        (aggregate != null &&
          ...
          isDvtSubstraitCountFunction(input.target.plan, aggregate) &&
          ordinal != null)),
...
export function windowTool(input: SelectedRelationInput): CanvasRelationalOperatorTool {
  ...
  const ordinal = readDvtSubstraitFieldReferenceOrdinal(window?.sorts[0]?.expr);
  return {
    id: 'window',
    active: input.intent === 'edit',
    fields,
    enabled:
      ...
      (input.intent === 'insert' ||
        (project?.expressions.length === 1 &&
          ...
          ordinal != null &&
          isDvtSubstraitRowNumberFunction(input.target.plan, window))),
    fieldId: fields.find((field) => field.ordinal === ordinal)?.fieldId,
    alias:
      window == null
        ? undefined
