...
describe('generic Substrait projection identity', () => {
  it('allocates opaque relation and source-field identities without changing caller-owned outputs', () => {
    ...
    const sourceBinding = projectionDraft.sidecar.relations.find((relation) => relation.sourceRef != null);
    const targetBinding = projectionDraft.sidecar.relations.find((relation) => relation.sourceRef == null);
    if (sourceBinding == null || targetBinding == null) throw new Error('Expected two relation bindings.');
  ...
  it('treats legacy relation and source-field strings as opaque persisted identity', () => {
    ...
    if (sourceBinding == null || targetBinding == null) throw new Error('Expected relation bindings.');
