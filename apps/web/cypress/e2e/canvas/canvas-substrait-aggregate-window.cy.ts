/** Owned concern: prove grouping and ranking composition through the governed Canvas draft rail. */
import { inspectDvtSubstraitPilotAggregateWindowDraft } from '../../../src/app/views/canvas/canvasDvtSubstraitAggregateWindow';
...
describe('Canvas Substrait aggregate and window composition', () => {
  beforeEach(() => {
    stubRuntimeCapabilities();
    stubStatefulCanvasDraftAuthoring({
      substraitPilot: true,
      title: 'Substrait grouped ranking',
    });
  });

  it('groups, ranks, persists and reloads one canonical revision', () => {
    visitCanvas();
    ...
    cy.get('[data-slot="dvt-substrait-aggregation-authoring"]').should('be.visible');
    cy.get('[data-slot="dvt-substrait-aggregate-window-order-readonly"]').should(
      'contain.text',
      'customer_count'
    );
    cy.get('button[data-slot="dvt-substrait-apply-aggregate-window"]')
      .focus()
      .should('have.focus')
      .then(() => cy.press(Cypress.Keyboard.Keys.ENTER));

    cy.get('[data-slot="dvt-substrait-aggregate-window-authoring"]')
      .should('be.visible')
      .and('have.attr', 'data-capability-id')
      .and('contain', 'row_number');
    cy.get('input[data-slot="dvt-substrait-aggregate-window-output-name"]')
      .clear()
      .type('country_rank')
      .blur();
    ...
    cy.wrap(null).should(() => {
      ...
      const inspection = inspectDvtSubstraitPilotAggregateWindowDraft(
    ...
    cy.get('input[data-slot="dvt-substrait-aggregate-window-output-name"]').should(
      'have.value',
      'country_rank'
    );
  });
