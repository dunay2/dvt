...
const ALL_EXTERNAL_COMPILE_ADAPTERS: readonly Provider[] = ['conductor', 'mock', 'temporal'];

export const BUILT_IN_EXTERNAL_COMPILE_FAMILIES = ['sql_transform', 'spark'] as const;
export type ExternalCompileFamilyId = (typeof BUILT_IN_EXTERNAL_COMPILE_FAMILIES)[number];

export const BUILT_IN_EXTERNAL_COMPILE_STEP_KINDS = [
  'PREPARE_POSTGRES_TRANSFORM',
  'POSTGRES_SQL_TRANSFORM',
  'CAPTURE_MATERIALIZATION_EVIDENCE',
  'SPARK_JOB',
] as const;
export type ExternalCompileStepKind = (typeof BUILT_IN_EXTERNAL_COMPILE_STEP_KINDS)[number];

export interface StepFamilyDefinition {
  readonly family: ExternalCompileFamilyId;
...
export interface ResolvedStepCatalog {
  readonly families: ReadonlyMap<ExternalCompileFamilyId, StepFamilyDefinition>;
  readonly stepKinds: ReadonlyMap<ExternalCompileStepKind, StepKindDefinition>;
}

const DEFAULT_EXECUTION_PROFILE: StepKindExecutionProfile = {
  supportedAdapters: ALL_EXTERNAL_COMPILE_ADAPTERS,
  requiredCapabilities: [],
};

type ExternalCompileStepSchema =
...
export interface StepKindDefinition {
  readonly kind: ExternalCompileStepKind;
  readonly family: ExternalCompileFamilyId;
  readonly schema: ExternalCompileStepSchema;
...
export function resolveExternalCompileCatalog(options?: {
...
): ReadonlyMap<ExternalCompileFamilyId, StepFamilyDefinition> {
  const families = new Map<ExternalCompileFamilyId, StepFamilyDefinition>();
  for (const definition of definitions) {
    if (families.has(definition.family)) {
      throw new Error(`Duplicate external compile family definition: ${definition.family}`);
    }
...
function buildStepKindMap(
  ...
  families: ReadonlyMap<ExternalCompileFamilyId, StepFamilyDefinition>
): ReadonlyMap<ExternalCompileStepKind, StepKindDefinition> {
  const stepKinds = new Map<ExternalCompileStepKind, StepKindDefinition>();
  for (const definition of definitions) {
    if (!families.has(definition.family)) {
      throw new Error(
        `External compile step kind ${definition.kind} references unknown family ${definition.family}`
      );
    }
    if (stepKinds.has(definition.kind)) {
      throw new Error(`Duplicate external compile step kind definition: ${definition.kind}`);
    }
