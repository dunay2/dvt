-- Harden the Canvas node workbench panel reversal after a concurrent/local
-- refresh reintroduced DB-only migrations and a feature-mechanization rail for
-- missing CanvasNodeWorkbenchPanel files.
...
values (
  ...
  'Harden untracked Canvas node workbench panel reversal',
  'Architecture / Planning DB / Frontend',
  'implemented',
  'A local refresh reintroduced DB-only CanvasNodeWorkbenchPanel ownership and feature-mechanization rows after the first reversal. The filesystem still has no CanvasNodeWorkbenchPanel source or test file, so this migration removes the stale local rail, ownership, relations, ports, and orphan schema_migration records again after the reintroduced data.',
  'boundary_drift',
...
set
  status = 'superseded',
  rationale = 'Superseded by PLANNING-DB-WEB-CANVAS-NODE-WORKBENCH-PANEL-UNTRACKED-REACTIVATION-HARDENING-20260619 because its migration files are not tracked and its declared CanvasNodeWorkbenchPanel files do not exist.',
  updated_at = now()
where design_id in (
...
values
  ...
  (
    'SYS-WEB-CANVAS-NODE-WORKBENCH-PANEL',
    'governance_ref',
    'tools/planning-db/migrations/233_harden_untracked_canvas_node_workbench_panel_reversal.sql',
    0
  ),
  (
    'SYS-WEB-CANVAS-NODE-WORKBENCH-PANEL',
  ...
  (
    'SYS-WEB-CANVAS-NODE-WORKBENCH-PANEL',
    'transition',
    'Reactivation remains superseded until tracked CanvasNodeWorkbenchPanel source and test files exist.',
    0
  )
...
set
  ...
  source_path = 'tools/planning-db/migrations/233_harden_untracked_canvas_node_workbench_panel_reversal.sql',
  source_content_sha256 = coalesce(
    (
      select file_ref.content_hash
      from planning_query_store.governance_files file_ref
      where file_ref.path = 'tools/planning-db/migrations/233_harden_untracked_canvas_node_workbench_panel_reversal.sql'
      limit 1
    ),
...
set
  signal_name = 'Superseded Canvas node workbench panel reactivation is observable through component-profile, files query absence, source-drift, and migration evidence.',
  required = true,
  status = 'implemented'
