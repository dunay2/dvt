create table if not exists planning_query_store.risk_debt_items (
  ...
  source_path text not null references planning_query_store.governance_files(path) on delete cascade,
  title text not null,
  ...
  raw_debt jsonb not null default '{}'::jsonb,
  imported_at timestamptz not null default now()
);

create index if not exists risk_debt_items_priority_idx
  on planning_query_store.risk_debt_items(priority, status);

create index if not exists risk_debt_items_component_idx
  on planning_query_store.risk_debt_items(component_unit);

create or replace view planning_query_store.risk_debt_query as
select
  risk_id,
  source_path,
  title,
