-- Revert the local DB-only Canvas node workbench panel reactivation that was
-- applied without matching migration files in this branch. The filesystem has
-- no CanvasNodeWorkbenchPanel source or test file; the active overlay renders
...
values (
  ...
  'Reverse untracked Canvas node workbench panel reactivation',
  'Architecture / Planning DB / Frontend',
  'implemented',
  'The local Planning DB recorded 225_web_canvas_node_workbench_panel_reactivation and 226_web_canvas_node_workbench_panel_effective_reactivation as applied, but those migration files are not tracked in this branch and the declared CanvasNodeWorkbenchPanel.tsx files do not exist. This migration removes the DB-only file ownership, retires the panel relations and ports, and restores the panel component as audit-only deprecated evidence.',
  'boundary_drift',
  'RecordArchitectureComponent;RecordArchitectureRelation;ValidateFilesystemCoverage;ValidateComponentIntegrity',
...
set
  status = 'superseded',
  rationale = 'Superseded by PLANNING-DB-WEB-CANVAS-NODE-WORKBENCH-PANEL-UNTRACKED-REACTIVATION-REVERSAL-20260619 because its migration file is not tracked and its declared CanvasNodeWorkbenchPanel files do not exist.',
  updated_at = now()
where design_id = 'PLANNING-DB-WEB-CANVAS-NODE-WORKBENCH-PANEL-EFFECTIVE-REACTIVATION-20260619';
...
values
  ...
  (
    'SYS-WEB-CANVAS-NODE-WORKBENCH-PANEL',
    'governance_ref',
    'tools/planning-db/migrations/232_revert_untracked_canvas_node_workbench_panel_reactivation.sql',
    0
  ),
  (
    'SYS-WEB-CANVAS-NODE-WORKBENCH-PANEL',
  ...
  (
    'SYS-WEB-CANVAS-NODE-WORKBENCH-PANEL',
    'transition',
    'Reactivation is superseded until tracked CanvasNodeWorkbenchPanel source and test files exist.',
    0
  )
...
set
  ...
  cq_rails = 'none - superseded untracked reactivation',
  source_path = 'tools/planning-db/migrations/232_revert_untracked_canvas_node_workbench_panel_reactivation.sql',
  source_content_sha256 = coalesce(
    (
      select file_ref.content_hash
      from planning_query_store.governance_files file_ref
      where file_ref.path = 'tools/planning-db/migrations/232_revert_untracked_canvas_node_workbench_panel_reactivation.sql'
      limit 1
    ),
...
set
  responsibility = 'Superseded audit-only component retained to document a reverted local DB reactivation.',
...
set
  signal_name = 'Superseded Canvas node workbench panel reactivation is observable through component-profile, files query absence, and migration evidence.',
  required = true,
  status = 'implemented'
