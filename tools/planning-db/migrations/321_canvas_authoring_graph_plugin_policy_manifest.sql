-- DB-first local overlay for the AuthorCanvasGraphEdge feature manifest.
-- The old role-table authority has been retired in favor of plugin/capability
-- connection policy from apps/web/src/app/plugins/contracts/ConnectionRules.ts.
  ...
  select
    ...
    jsonb_build_array(
      jsonb_build_object(
        ...
        'fowlerSignals', jsonb_build_array('Replace Conditional with Strategy', 'Explicit Policy'),
        'architectureGuard',
      ...
      jsonb_build_object(
        'name', 'evaluatePluginConnectionRules',
        'path', 'apps/web/src/app/plugins/contracts/ConnectionRules.ts',
        'dddOwner', 'Plugin connection policy',
        'cqRails', jsonb_build_array('AuthorCanvasGraphEdge'),
        'fowlerSignals', jsonb_build_array('Replace Conditional with Strategy', 'Explicit Policy'),
      ...
      jsonb_build_object(
        'name', 'formatCanvasConnectionRejection',
        'path', 'apps/web/src/app/views/canvas/canvasCopyFormatting.ts',
        'dddOwner', 'Canvas authoring copy policy',
        'cqRails', jsonb_build_array('AuthorCanvasGraphEdge'),
        'fowlerSignals', jsonb_build_array('Consolidate Conditional Expression'),
        'architectureGuard',
          'pnpm --filter @dvt/web exec vitest run src/app/views/canvas/useCanvasGraphHandlers.edgeAuthoring.test.tsx',
...
merged_implementation_refs as (
  ...
  from (
    ...
    select jsonb_build_object(
      'name', 'Canvas authoring graph plugin policy manifest',
      'path', 'tools/planning-db/migrations/321_canvas_authoring_graph_plugin_policy_manifest.sql',
