...
related_test_component_links as (
  ...
  join planning_query_store.governance_component_query test_component
    on test_component.parent_id = source.parent_id
   and test_component.component_id <> source.component_id
  where regexp_replace(lower(test_component.component_id), '-tests?$', '') =
        regexp_replace(lower(source.component_id), 's$', '')
),
related_test_components as (
  select
    component_id,
    count(*)::int as test_component_count,
    coalesce(
      jsonb_agg(
        jsonb_build_object(
...
related_test_file_sets as (
  select
    link.component_id,
    count(file.path)::int as related_test_file_count,
    coalesce(jsonb_agg(file.path order by file.path), '[]'::jsonb) as related_test_files
  from related_test_component_links link
  join planning_query_store.governance_component_files file
    on file.component_id = link.test_component_id
  ...
  jsonb_strip_nulls(
    jsonb_build_object(
      ...
      'tests', jsonb_build_object(
        'testComponents', coalesce(test_components.test_components, '[]'::jsonb),
        'testFiles',
          coalesce(files.test_files, '[]'::jsonb) ||
          coalesce(related_test_files.related_test_files, '[]'::jsonb),
        'testFileCount',
          coalesce(files.test_file_count, 0) +
          coalesce(related_test_files.related_test_file_count, 0),
        ...
        to_jsonb(
          array_remove(
            array[
              ...
              case
                when coalesce(files.test_file_count, 0) +
                     coalesce(related_test_files.related_test_file_count, 0) = 0
...
left join related_test_components test_components
  on test_components.component_id = component.component_id
left join component_file_sets files
  on files.component_id = component.component_id
left join related_test_file_sets related_test_files
  on related_test_files.component_id = component.component_id
