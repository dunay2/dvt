-- Keep planned Canvas UX backlog boundaries from masquerading as implemented
-- architecture. component_integrity_query should still fail active architecture
-- components that lack maturity evidence, but proposed/review-only virtual
-- Canvas surfaces must remain backlog until their implementation slices add
-- real component tests and observability.
...
values (
  'CANVAS-BACKLOG-INTEGRITY-READINESS-20260626',
  'E-CANVAS-UXDB-COMPONENT-SLICES-1',
  'Canvas backlog component integrity readiness projection',
  'Architecture / Planning DB',
  'review',
  'Canvas UX DB-first backlog rows include virtual component boundaries that are intentionally not implemented yet. Integrity must not require maturity tests for proposed/review backlog components because that incentivizes fake evidence; active approved/implemented/drift components still fail when required maturity evidence is missing.',
  'hidden_authority',
 ...
 WITH architecture_components AS MATERIALIZED (
         ...
         SELECT 'missing_maturity_evidence'::text AS finding_kind,
          ...
          WHERE COALESCE(array_length(maturity.missing_reasons, 1), 0) > 0
            AND component.status in ('approved', 'implemented', 'drift')
...
set
  source_path = 'tools/planning-db/migrations/291_canvas_backlog_integrity_readiness_projection.sql',
  source_content_sha256 =
    md5('E-CANVAS-UXDB-COMPONENT-SLICES-1:RenderCanvasShellMainPanelFrame:291')
    || md5('canvas-backlog-integrity-readiness'),
  ...
  architecture_guards = jsonb_build_array(
    'node --test --test-name-pattern "tracked migrations keep planned Canvas backlog components out of maturity blockers" scripts/planning-db-migrate.test.cjs',
    'pnpm planning:db:integrity:check',
    'pnpm verify:prepush'
  ),
  ...
  raw_rail = raw_rail || jsonb_build_object(
    ...
    '291_canvas_backlog_integrity_readiness_projection'
  ),
  raw_manifest = raw_manifest || jsonb_build_object(
    ...
    'backlogIntegrityReadinessReconciledBy',
    '291_canvas_backlog_integrity_readiness_projection'
  ),
  revision = revision + 1,
  updated_at = now()
where rail_id = 'local#E-CANVAS-UXDB-COMPONENT-SLICES-1#query#rendercanvasshellmainpanelframe'
  and source_path = 'planning-db:task/E-CANVAS-UXDB-COMPONENT-SLICES-1';
