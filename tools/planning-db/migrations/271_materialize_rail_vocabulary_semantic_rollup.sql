-- Keep the vocabulary dashboard bounded by forcing the semantic duplicate
-- rollup to materialize once. Without this fence, PostgreSQL can inline the
-- aggregate and re-run it for each rail row when dashboard reads sort/limit the
-- unioned findings.
