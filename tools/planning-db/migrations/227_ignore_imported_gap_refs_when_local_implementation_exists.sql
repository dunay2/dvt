-- A local implemented rail reconciliation should not be counted as an exact
-- duplicate with an imported documentation-only gap for the same rail. Keep
-- duplicate detection strict for multiple active implementations, but ignore
-- imported gap references once a non-gap canonical implementation exists.
