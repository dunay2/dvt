-- Retire the imported command_query_rails rows for obsolete generic SourceImport
-- aliases. Migration 563 retired the local feature-mechanization projection; this
-- follow-up retires the imported catalog rows that command-query-rails exposes.
...
set
  ...
  rail_status = 'retired',
  source_path = 'tools/planning-db/migrations/564_retire_imported_source_import_generic_alias_rails.sql',
  source_content_sha256 = md5('imported-source-import-generic-alias-rail-retirement:564:' || target_rails.rail_name),
  ...
  architecture_guards = jsonb_build_array(
    ...
    'pnpm planning:db:query creation-intent --intent "import source objects" must prefer ImportWarehouseSources or mark ImportSourceObjects retired'
  ),
  ...
  raw_rail = (
    ...
    || jsonb_build_object(
      'status', 'retired',
      'retirementReason', 'Imported generic SourceImport rail alias superseded by the implemented Warehouse source-import rail.',
