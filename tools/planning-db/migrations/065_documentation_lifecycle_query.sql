...
stateful as (
  select
    ...
    case
      ...
      when classified.canonicality = 'canonical' then 'active'
      else 'indexed'
...
values (
  'Documentation lifecycle catalog',
  'knowledge_documents, knowledge links, action rows, repository backrefs, and lifecycle projection',
  'Git edit followed by pnpm governance:refresh or pnpm governance:db:import',
  'import',
  'pnpm planning:db:query documentation-lifecycle',
  ...
  0,
  'migration',
  '{"authority":"logical_projection","writer":"git_import"}'::jsonb
)
