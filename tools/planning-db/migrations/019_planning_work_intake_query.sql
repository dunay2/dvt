...
select
  ...
  'pnpm planning:db:query task-trace --task ' || task.task_id || ' --limit 30' as suggested_query,
...
select
  ...
  case
    when gap.task_id is not null
      then 'pnpm planning:db:query task-trace --task ' || gap.task_id || ' --limit 30'
    when gap.document_path is not null
      then 'pnpm planning:db:query task-gaps --path ' || gap.document_path || ' --limit 30'
    else 'pnpm planning:db:query task-gaps --kind ' || gap.gap_kind || ' --limit 30'
...
select
  ...
  action.reason,
  'pnpm planning:db:query docs-disposition --kind ' || action.action_kind
    || ' --path ' || action.document_path || ' --limit 30' as suggested_query,
...
select
  ...
  remediation.reason,
  'pnpm planning:db:query remediation --kind ' || remediation.task_type
