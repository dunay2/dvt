-- Rehydrate the DB-local feature mechanization manifest for the Canvas node
-- workbench panel after governance imports. The implementation guard reads
-- these manifests from Planning DB, so the real panel symbols must remain
-- declared locally after command/query rail imports are refreshed.
