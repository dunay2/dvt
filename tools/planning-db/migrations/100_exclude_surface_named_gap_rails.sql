-- Keep vocabulary findings mutually exclusive after surfacing gap rails with
-- transport-shaped names. A rail named after an API/UI/CLI/worker/adapter
-- surface is still historical vocabulary debt, but it should be counted in the
-- surface bucket rather than also inflating generic gap debt.
