-- Reconcile Source Import wizard step file ownership after the live proof and
-- review-template hardening. WarehouseConnectionCreateForm.tsx is a single
-- step-owned component file. It must not remain visible through the older
-- SourceImportDialog aggregate or through a second role under the same owner.
...
values (
  ...
  jsonb_build_object(
    'retiredForSourceImportStepOwnership', true,
    'reassignedToComponent', 'SYS-WEB-CANVAS-SOURCE-IMPORT-WIZARD-STEPS',
    'reassignedRail', 'CreateWarehouseConnection',
    'reason', 'The connection creation form is owned by the Source Import wizard steps leaf, not by the dialog aggregate.'
  ),
  'tools/planning-db/migrations/557_source_import_wizard_step_file_ownership_dedup.sql',
  md5('retire:SourceImportDialog:WarehouseConnectionCreateForm:557')
)
...
values (
  ...
  'EV-SOURCE-IMPORT-WIZARD-STEP-FILE-OWNERSHIP-DEDUP',
  'architecture-test',
  'current',
  'pnpm planning:db:query frontend-component-files --path apps/web/src/app/components/sourceImportWizard/WarehouseConnectionCreateForm.tsx --limit 80',
  'CreateWarehouseConnection',
  'source-import-wizard-step-file-ownership-dedup',
  'WarehouseConnectionCreateForm.tsx has exactly one visible frontend component owner and one visible file role.',
  jsonb_build_object(
    'componentOwner', 'SYS-WEB-CANVAS-SOURCE-IMPORT-WIZARD-STEPS',
    'retiredOwner', 'web.component.canvas.SourceImportDialog',
    'retiredDuplicateRole', 'presentation',
    'path', 'apps/web/src/app/components/sourceImportWizard/WarehouseConnectionCreateForm.tsx'
  ),
  'tools/planning-db/migrations/557_source_import_wizard_step_file_ownership_dedup.sql',
  md5('EV-SOURCE-IMPORT-WIZARD-STEP-FILE-OWNERSHIP-DEDUP:557')
)
