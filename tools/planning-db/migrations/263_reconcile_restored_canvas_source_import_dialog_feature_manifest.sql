-- Reconcile the restored Canvas source import dialog feature manifest row.
-- Some local DBs already contain an applied 262 restore record that is not
-- present in the repository. This migration repairs that DB row without
-- reusing the applied 262 migration filename.
...
values (
  ...
  'Reconcile restored Canvas source import dialog feature manifest row',
  'Architecture / Planning DB / Frontend',
  'implemented',
  'A restored OpenCanvasSourceImportDialog local feature manifest can point to tools/planning-db/migrations/262_restore_canvas_source_import_dialog_feature_manifest.sql and still carry sparse raw_manifest.featureId plus removed CanvasSourceImportDialogHost symbols. This migration moves the row to a tracked 263 reconciliation source and converts it into retirement metadata so feature-mechanization validation reads the canonical governed plan instead.',
  'boundary_drift',
...
set
  ...
  raw_rail = coalesce(rail.raw_rail, '{}'::jsonb)
    || jsonb_build_object(
      ...
      '263_reconcile_restored_canvas_source_import_dialog_feature_manifest'
    ),
  raw_manifest = (
    ...
    'restoredFeatureManifestReconciledBy',
    '263_reconcile_restored_canvas_source_import_dialog_feature_manifest'
  ),
  symbol_refs = jsonb_build_array(
    'tools/planning-db/migrations/263_reconcile_restored_canvas_source_import_dialog_feature_manifest.sql#ReconcileRestoredCanvasSourceImportDialogFeatureManifest'
  ),
