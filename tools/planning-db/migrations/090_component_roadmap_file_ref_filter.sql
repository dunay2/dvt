...
planned_component_refs as (
  ...
  where nullif(btrim(component_ref.value), '') is not null
    ...
    and component_ref.value !~ '\.[A-Za-z0-9]+$'
    and (
...
values (
  ...
  'DB read model for implemented, declared, and planned component gap inspection; feature-mechanization documentation guides and file refs remain sources, not component ids',
  ...
  0,
  'migration',
  '{"authority":"db_projection","componentRefFilter":"mechanical-component-id-or-owned-directory"}'::jsonb
)
