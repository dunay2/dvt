...
with document_panels as (
  ...
  left join planning_query_store.component_roadmap_query roadmap
    on roadmap.source_path = lifecycle.document_path
  cross join lateral (
    values
...
document_section_panels as (
  ...
  left join planning_query_store.component_roadmap_query roadmap
    on roadmap.source_path = document.document_path
),
component_panels as (
  select
...
required_section_gaps as (
  ...
  left join planning_query_store.component_roadmap_query roadmap
    on roadmap.source_path = lifecycle.document_path
  where lifecycle.canonicality in ('canonical', 'proposal')
...
values (
  ...
  0,
  'migration',
  '{"authority":"db_projection","panelModel":"relational"}'::jsonb
)
