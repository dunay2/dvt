-- Final DB-first authority for the Canvas node workbench panel. This migration
-- intentionally runs after stale local DBs may have applied a now-absent
-- retirement guard, and restores both component ownership and feature
-- mechanization in one post-import authority pass.
...
values (
  ...
  'Restore Canvas node workbench panel authority after post-import legacy rows',
  'Architecture / Planning DB',
  'implemented',
  'The DB-first component profile must point at the real CanvasNodeWorkbenchPanel component, not the retired CanvasNodeWorkbenchDuplicateResolution placeholder.',
...
where component_id = 'SYS-WEB-CANVAS-NODE-WORKBENCH-PANEL'
  and (
    item_value ilike '%CanvasNodeWorkbenchDuplicateResolution%'
    or item_value ilike '%Superseded audit-only component%'
    or item_value ilike '%deprecated%'
    or item_value ilike '%retired%'
    or item_value = 'duplicate_semantics'
  );
...
values
  ...
  (
    'SYS-WEB-CANVAS-NODE-WORKBENCH-PANEL',
    'invariant',
    'CanvasNodeWorkbenchOverlay delegates node detail rendering to CanvasNodeWorkbenchPanel, never to CanvasInspectorPanel.',
    0
  ),
  (
    'SYS-WEB-CANVAS-NODE-WORKBENCH-PANEL',
...
-- Rehydrate the feature manifest in the same final authority migration.
