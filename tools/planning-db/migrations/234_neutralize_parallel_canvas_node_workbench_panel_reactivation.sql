-- Neutralize the parallel Canvas node workbench panel reactivation data without
-- deleting the external schema_migrations rows. Keeping those local records in
-- place prevents a concurrent migrator that owns the missing files from
-- immediately reapplying the stale component data.
...
values (
  ...
  'Neutralize parallel Canvas node workbench panel reactivation',
  'Architecture / Planning DB / Frontend',
  'implemented',
  'A concurrent migrator can reapply local-only CanvasNodeWorkbenchPanel migrations when their schema_migrations rows are removed. This migration neutralizes the stale component data while intentionally leaving those external migration records in place until the parallel branch is reconciled.',
  'boundary_drift',
  'RecordArchitectureComponent;RecordArchitectureRelation;ValidateRailVocabulary;ValidateSourceDrift',
...
set
  status = 'superseded',
  rationale = 'Superseded by PLANNING-DB-WEB-CANVAS-NODE-WORKBENCH-PANEL-PARALLEL-REACTIVATION-NEUTRALIZATION-20260619 because the active filesystem still has no CanvasNodeWorkbenchPanel source or test files.',
  updated_at = now()
where design_id in (
...
values
  ...
  (
    'SYS-WEB-CANVAS-NODE-WORKBENCH-PANEL',
    'governance_ref',
    'tools/planning-db/migrations/234_neutralize_parallel_canvas_node_workbench_panel_reactivation.sql',
    0
  ),
  (
    'SYS-WEB-CANVAS-NODE-WORKBENCH-PANEL',
  ...
  (
    'SYS-WEB-CANVAS-NODE-WORKBENCH-PANEL',
    'transition',
    'Parallel reactivation remains neutralized until tracked CanvasNodeWorkbenchPanel source and test files exist.',
    0
  )
...
set
  ...
  cq_rails = 'none - superseded parallel reactivation',
  source_path = 'tools/planning-db/migrations/234_neutralize_parallel_canvas_node_workbench_panel_reactivation.sql',
  source_content_sha256 = coalesce(
    (
      select file_ref.content_hash
      from planning_query_store.governance_files file_ref
      where file_ref.path = 'tools/planning-db/migrations/234_neutralize_parallel_canvas_node_workbench_panel_reactivation.sql'
      limit 1
    ),
...
set
  responsibility = 'Superseded audit-only component retained to document a neutralized parallel DB reactivation.',
...
set
  signal_name = 'Neutralized Canvas node workbench panel reactivation is observable through component-profile, files query absence, source-drift, and migration evidence.',
  required = true,
  status = 'implemented'
