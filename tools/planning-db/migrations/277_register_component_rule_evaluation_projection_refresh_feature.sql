...
-- component rule-evaluation refresh helper. Component drift and quality reads
-- are priority operator queries, so their heavy rule evaluation inputs must be
-- refreshed as DB projections during governance import.
      ...
      from (
        ...
        from jsonb_array_elements_text(
          target.symbol_refs || jsonb_build_array(
            'scripts/planning-db-import.cjs#refreshComponentRuleEvaluationMaterializedProjection',
            'tools/planning-db/migrations/276_component_rule_evaluation_priority_projection.sql#component_engineering_rule_evaluation_projection',
            'tools/planning-db/migrations/276_component_rule_evaluation_priority_projection.sql#component_engineering_drift_query',
            'tools/planning-db/migrations/276_component_rule_evaluation_priority_projection.sql#component_engineering_quality_query'
          )
      ...
      from (
        ...
        from jsonb_array_elements_text(
          target.implementation_refs || jsonb_build_array(
            ...
            'tools/planning-db/migrations/276_component_rule_evaluation_priority_projection.sql',
            'tools/planning-db/migrations/277_register_component_rule_evaluation_projection_refresh_feature.sql'
          )
  ...
  select
    ...
    case
      when exists (
        ...
        where symbol->>'name' = 'refreshComponentRuleEvaluationMaterializedProjection'
      ) then coalesce(patch.manifest->'symbols', '[]'::jsonb)
      else coalesce(patch.manifest->'symbols', '[]'::jsonb) || jsonb_build_array(
        jsonb_build_object(
          'name', 'refreshComponentRuleEvaluationMaterializedProjection',
          'path', 'scripts/planning-db-import.cjs',
          'dddOwner', 'PlanningDbComponentRuleEvaluationReadModel',
          'cqRails', jsonb_build_array('RefreshComponentRuleEvaluationProjection'),
          'fowlerSignals', jsonb_build_array('slow_query_projection', 'component_drift_query'),
    ...
    coalesce(patch.manifest->'redGreenCycles', '[]'::jsonb) || jsonb_build_array(
      jsonb_build_object(
        'id', 'refresh-materialized-component-rule-evaluations-after-import',
        'redTest',
        'node --test --test-name-pattern "planning DB import refreshes the materialized component rule evaluation projection" scripts/planning-db-import.test.cjs',
        'expectedFailure',
        'Governance import updates component ownership and rule inputs without refreshing the materialized rule evaluation projection used by component drift reads.',
...
set
  ...
  source_path = 'tools/planning-db/migrations/277_register_component_rule_evaluation_projection_refresh_feature.sql',
          ...
          jsonb_set(
            jsonb_set(
              manifest_patch.manifest || jsonb_build_object(
                'implementationPlan',
                'Planning DB materialized diagnostics refresh code-symbol, component file-ownership, and component rule-evaluation projections during governance import so priority integrity and drift reads do not recompute local ownership and rule predicates.',
                'componentRuleEvaluationProjectionRegisteredBy',
                '277_register_component_rule_evaluation_projection_refresh_feature'
              ),
