...
import { computeBooleanScope, matchesAnyPattern, PR_QUALITY_SCOPE_PATTERNS } from '../scope-config.mjs';

test('matchesAnyPattern supports wildcard and normalizes Windows separators', () => {
  assert.ok(matchesAnyPattern('packages\\@dvt\\adapter-postgres\\src\\index.ts', ['packages/@dvt/adapter-postgres/**']));
  assert.ok(matchesAnyPattern('tsconfig.base.json', ['tsconfig*.json']));
  assert.ok(!matchesAnyPattern('packages/@dvt/adapter-postgresx/src/index.ts', ['packages/@dvt/adapter-postgres/**']));
});

test('computeBooleanScope marks adapter_postgres_changed for relevant workflow/config changes', () => {
  const fromWorkflow = computeBooleanScope(['.github/workflows/pr-quality-gate.yml'], PR_QUALITY_SCOPE_PATTERNS);
