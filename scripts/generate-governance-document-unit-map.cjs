...
const outputYamlPath = path.join(
  repoRoot,
  'docs',
  'planning',
  'status',
  'system-governance-document-unit-map.docs.yaml'
);
...
function getTrackedDocs() {
  ...
  const generatedMarkdownPath = toPosix(path.relative(repoRoot, outputMarkdownPath));
  if (fs.existsSync(outputMarkdownPath) && !trackedDocs.includes(generatedMarkdownPath)) {
    trackedDocs.push(generatedMarkdownPath);
  }
