...
function gitChangedFiles() {
  try {
    const out = execSync(
      'git rev-parse --abbrev-ref --symbolic-full-name @{u} >/dev/null 2>&1 && git diff --name-only origin/main...HEAD || git diff --name-only HEAD~1..HEAD',
...
if (prettierFiles.length) {
  ...
  const res = spawnSync('prettier', ['--check', ...prettierFiles], {
    stdio: 'inherit',
    shell: true,
  });
...
if (eslintFiles.length) {
  ...
  const res = spawnSync('eslint', ['--max-warnings', '0', ...eslintFiles], {
    stdio: 'inherit',
    shell: true,
  });
  if (res.status !== 0) process.exit(res.status);
