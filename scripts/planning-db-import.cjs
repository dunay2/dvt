...
function buildCommandQueryRailSnapshot(options = {}) {
  ...
  rails.push(...extractDocumentedRailRows(referenceDocuments));
...
function readTrackedFileSources(gitPathspecs) {
  return readTrackedDocumentPaths(gitPathspecs).map((sourcePath) => {
    const raw = fs.readFileSync(path.join(repoRoot, sourcePath), 'utf8');
    return { sourcePath, raw, contentSha256: sha256(raw) };
  });
...
function listTrackedCommandQuerySourceFiles() {
  return readTrackedFileSources(['apps/**', 'packages/**', 'scripts/**', 'tools/**']).filter(
