...
exports.SequenceClock = void 0;
exports.parseIsoUtcToEpochMs = parseIsoUtcToEpochMs;
exports.epochMsToIsoUtc = epochMsToIsoUtc;
...
function assertInRange({ value, min, max, label, iso }) {
...
function validateIsoParts(parts, iso) {
...
function parseIsoUtcToEpochMs(iso) {
...
function epochMsToIsoUtc(ms) {
  ...
  rem %= MS_PER_SECOND;
  const milli = rem;
  return `${pad(year, 4)}-${pad(month, 2)}-${pad(day, 2)}T${pad(hour, 2)}:${pad(min, 2)}:${pad(sec, 2)}.${pad(milli, 3)}Z`;
}
/**
 ...
 */
class SequenceClock {
  constructor(baseIsoUtc = '2026-02-12T00:00:00.000Z') {
    this.counter = 0;
...
exports.SequenceClock = SequenceClock;
