/**
 * Owned concern: Delivery-owned in-memory outbox state machine.
 *
 * Owns local/test claim eligibility, retry backoff, dead-letter, replay, shard,
 * and tenant/run ordering semantics for components that implement IOutboxStorage
 * without a durable adapter.
 */
...
export class InMemoryOutboxStorageCore implements IOutboxStorage {
  ...
  constructor(deps?: { nowMs?: () => number; shardCount?: number }) {
    this.nowMs = deps?.nowMs ?? (() => InMemoryOutboxStorageCore.EPOCH_MS);
  ...
  async listPendingForClaim(
    ...
    eligible.sort(InMemoryOutboxStorageCore.compareEligibleRecords);
