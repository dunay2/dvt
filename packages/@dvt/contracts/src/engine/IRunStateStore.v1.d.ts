...
export type EventType = 'RunQueued' | 'RunStarted' | 'RunPaused' | 'RunResumed' | 'RunCancelRequested' | 'RunCancelled' | 'RunCompleted' | 'RunFailed' | 'StepStarted' | 'StepCompleted' | 'StepFailed' | 'StepSkipped';
...
export type StepEventInput = RunEventInputBase & {
    stepId: string;
};
export type RunEventInput = RunEventInputBase & {
    stepId?: never;
};
...
export interface IRunStateStore {
    ...
    listEvents(tenantId: string, runId: string, options?: ListEventsOptions): Promise<EventEnvelope[]>;
    /**
