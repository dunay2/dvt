...
The following APIs are **forbidden** inside `packages/engine/src/` and `packages/adapter-temporal/src/workflows/`:
...
| -------------- | ------------------------------------------------- | -------------------------------------------------------------------------------- |
| Engine source  | `packages/engine/src/**/*.ts`                     | `new Date()`, `Date.now()`, `Math.random()`, `process.env`, `crypto.randomBytes` |
