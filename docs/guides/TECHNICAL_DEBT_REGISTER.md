...
| TD-0006 | DAG dependency field (`dependsOn`) implemented only in adapter-local plan typing                         | code    | packages/adapter-temporal + shared contracts | 2026-02-17      | Medium | Medium | open    | Contracts + Temporal maintainers      | Promote `dependsOn` to canonical shared plan contract and propagate validation/docs across engine and adapters.                               | Avoid contract drift between workflow interpreter and shared plan schema | `packages/adapter-temporal/src/engine-types.ts`, `packages/adapter-temporal/src/workflows/RunPlanWorkflow.ts`, `packages/engine/src/contracts/executionPlan.ts`                                                                                  | 2026-02-17        |
