...
`IManagedObservability` extends that port only at resource-owning composition
roots so Fastify shutdown drains the batch processor without making lifecycle
methods part of ordinary application-service dependencies.
...
failure cannot change a successful callback or its domain result. The adapter
normalizes collector endpoints in linear time and does not duplicate exception
events already recorded by the observed operation. API composition tests prove
that application close invokes managed observability shutdown. Existing no-op
composition remains selected when observability is disabled and implements the
same lifecycle contract without allocating exporter resources.
