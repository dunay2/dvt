...
| -------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Transport and composition  | `buildApp()` bootstraps Fastify, public ops routes, protected runtime routes, request tracing, and operational hooks. Protected runtime registration is OIDC-gated.                            | `apps/api/src/app.ts`, `apps/api/src/routes/health.ts`, `apps/api/src/modules/registerOperationalHooks.ts`                                                                                                                                                      |
| Application command path   | `POST /runs/start` is translated into a start-run facade, authorization, duplicate probing, admission checks, optional planner execution, plan validation, and engine dispatch.                | `apps/api/src/entrypoints/http/startRunRoute.ts`, `apps/api/src/application/services/startRunAuthorizedFacade.ts`, `apps/api/src/application/services/BackpressureAwareStartRunUseCase.ts`, `apps/api/src/application/services/PlannerBackedStartRunUseCase.ts` |
