...
- Engine contracts and related documentation under `docs/architecture/components/engine/contracts/**`
...
- `scripts/validate-glossary-usage.cjs` MUST:
  1. Parse `docs/architecture/components/engine/contracts/engine/GlossaryContract.v1.md` and extract canonical terms.
...
- `scripts/generate-contract-index.cjs` MUST:
  1. Scan `docs/architecture/components/engine/contracts/**/*.v*.md`.
  2. Extract contract metadata (`status`, `version`, etc.).
  3. Regenerate `docs/architecture/components/engine/contracts/README.md` matrix deterministically.
