# Engine Capabilities

## Purpose

Summarizes the capability-related assets used by the engine component and points
to the canonical capability definitions.

## Canonical capability assets

- [Capabilities guide](./README.md)
- [Capabilities schema](./capabilities.schema.json)
- [Adapter capabilities matrix](./adapters.capabilities.json)
- [Validation report schema](./validation-report.schema.json)

## Related contracts

- [Provider adapter contract](../engine/IProviderAdapter.v1.md)
- [Workflow engine interface](../engine/IWorkflowEngine.v1.md)

## Navigation

- [Engine component home](../../index.md)
- [Core](../../architecture/core.md)
- [Adapters](../../adapters/index.md)
- [Workflows](../../architecture/workflows.md)
- [Security](../../security/index.md)
- [Operations](../../ops/index.md)
- [Contracts](../index.md)
- [Canonical C4 architecture](../../architecture/c4-engine.md)
