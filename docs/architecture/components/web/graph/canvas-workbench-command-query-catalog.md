...
| `SetOperationalDrawerTabVisibility`   | command | accepted | Canvas workbench presentation | `OperationalDrawerTabVisibility`     |
...
| `FocusCanvasGraphNode`                | command | accepted | Canvas viewport presentation  | `CanvasViewportNodeFocus`            |
...
| `CreateCanvasEdge`                    | command | accepted | Canvas workbench authoring    | `CanvasAuthoringGraph`               |
...
| `SetOperationalDrawerTabVisibility`   | Hide one operational drawer tab or restore one hidden tab while preserving its contributed panel state.                                                                      | tab id, visibility intent, ordered contributed tabs                                                                       | updated `OperationalDrawerTabVisibility` and deterministic active tab                        | operational drawer command port           | `useOperationalDrawerContributionStore`, `OperationalDrawerTabStrip`                                                                                        | route-local browser state; no graph, run, data, or contribution mutation                                                     | unknown tabs are ignored; closing the active tab selects an adjacent visible tab; every tab may be hidden; selecting or restoring a hidden tab reveals it; closing never deletes panel state                                                                      |
...
| `ConfigureCanvasViewportPreferences`  | Change Canvas background, grid visibility, grid size, grid color, snap-to-grid, and impact-overlay visibility preferences.                                                   | `CanvasViewportPreferences` value object                                                                                  | updated visual preference state                                                              | Canvas viewport preference command port   | `CanvasSettingsDialog`, `WorkbenchPropertiesWindow`, `uiLayoutStore`, `canvasInteractionStore`, `CanvasViewport`, `useCanvasOverlayModel`                   | route-local UI preference; no protected draft authority                                                                      | Cancel, close, and Escape dispatch no changes; invalid colors normalize; hidden grid does not disable dragging or node creation; snap changes coordinates only; disabling Impact skips impact traversal and decoration without disabling runtime or cost overlays |
| `FocusCanvasGraphNode`                | Center one rendered Canvas node from an explicit minimap or graph-search navigation intent.                                                                                  | node id, current rendered-node projection, selection capability                                                           | one-node viewport reveal plus optional selection changes                                     | Canvas viewport node-focus command port   | `canvasViewportNodeFocus.ts`, `useCanvasGraphSearchActivation.ts`, `CanvasViewport.tsx`, `CanvasViewportSurfaceView.tsx`                                    | route-local presentation only; no graph, coordinate, layout, backend, or protected-draft mutation                            | unknown or stale node ids produce no effects; read-only posture reveals without selection mutation; programmatic focus is not persisted as manual viewport movement; minimap background pan and zoom remain available                                             |
...
| `CreateCanvasEdge`                    | Admit and create one compatible dependency when the user completes a node-to-node connection gesture; the gesture is the command intent and requires no second confirmation. | React Flow connection, visible canonical nodes and edges, plugin port compatibility policy                                | accepted edge transaction and updated draft/session projection                               | Canvas edge authoring command seam        | `canvasConnectionAggregate.ts`, `canvasEdgeAdmissionTransaction.ts`, `useCanvasEdgeCommandRunner.ts`, `useCanvasEdgeAuthoringHandlers.ts`                   | graph edit permission plus active plugin connection policy                                                                   | missing endpoints, unsupported direction or node kinds, duplicate dependency, cycle, and read-only posture reject without creating an edge                                                                                                                        |
| `RemoveCanvasEdgeFromContext`         | Remove an editable edge selected by direct selection or an edge context-menu gesture through its existing semantic or graph edge-change lifecycle.                           | edge id and projected removal capability from the Canvas edge presentation                                                | semantic column command or `CanvasEdgeRemovalChange` consumed by `canvasGraphLifecycle.edge` | Canvas edge lifecycle command seam        | `canvasInteractionCommandSurface.ts`, `CanvasViewport.tsx`, `CanvasDependencyEdge.tsx`, `useCanvasControllerReadModel.ts`, `useCanvasEdgeChangeHandlers.ts` | graph edit permission plus the edge's projected removal capability                                                           | read-only posture and non-editable derived lineage cannot be deleted; unknown graph edge ids become lifecycle no-op through the existing React Flow edge-change path                                                                                              |
| `ConfigureCanvasDbtNode`              | Edit dbt export and round-trip metadata on the shared Source/Model profile without accepting projected SQL as model authority.                                               | selected `CanonicalNode`, `DbtNodeAuthoringMetadata`, connected-origin column catalog                                     | updated Canvas draft node override                                                           | Inspector and shared card command seam    | `CanvasInspectorAuthoringSection`, `DbtAuthoringFields`, `canvasDbtAuthoringModel`, `projectDbtModelArtifact`, graph-node column controls                   | current Canvas draft and active runtime policy                                                                               | blank required fields, unsupported materialization, unknown or duplicate selected columns, projected-SQL writes, and read-only posture reject; core Canvas cannot invent plugin-specific model-definition rules                                                   |
| `ConfigureCanvasDvtNode`              | Edit stable DVT Source binding/presentation, canonical Transform Substrait operations, and sink materialization metadata before preview.                                     | selected `CanonicalNode`, typed `DvtNodeAuthoringMetadata`, `CanvasColumnMappingSource`, and `CanvasColumnMappingTarget`  | updated Canvas draft node override                                                           | Inspector and card authoring command seam | `CanvasInspectorAuthoringSection`, focused DVT authoring modules, canonical projection commands, graph-node column controls                                 | current Canvas draft and active runtime policy                                                                               | Source `FilterRel`, removed SQL/VTX1 authority, unsupported expressions or plan shapes, incompatible or ambiguous mappings, unsupported sink policy, and read-only posture reject                                                                                 |
...
| `GenerateDbtWorkspaceArtifacts`       | Persist dbt project files by projecting the current graph definition into visible, read-only model SQL; generated SQL never becomes graph semantic authority.                | scoped dbt nodes and edges, normalized dbt metadata                                                                       | saved `dbt_project.yml`, model SQL, and `schema.yml`                                         | Workspace file-content command port       | `canvasDbtWorkspaceArtifacts`, `SaveWorkspaceFileContent`                                                                                                   | current workspace file write authorization                                                                                   | no model or missing connected origin rejects before plan preview; file write failure blocks preview; generated SQL must be visible but not editable in the plugin inspector; implicit SQL adoption rejects                                                        |
...
| Boundary drift: browser context menus, node gestures, and edge gestures could own graph intent.                   | `ResolveCanvasContextMenu`, `CreateCanvasAuthoringNode`, `CreateCanvasEdge`, `RemoveCanvasEdgeFromContext`, and existing node commands keep contextual actions behind named seams.      | Presentation Model plus Command Gateway. |
| Duplicate state: an admitted connection gesture was copied into pending modal state before the graph command ran. | `CreateCanvasEdge` treats the completed compatible gesture as the command intent and mutates through one aggregate transaction, while rejections remain fail-closed.                    | Command Gateway plus Remove Middle Man.  |
...
| Hidden authority: generated dbt code could live only in component memory or become an editable second model.      | `GenerateDbtWorkspaceArtifacts` writes plugin-generated model SQL through the workspace file command and exposes only a read-only projection; it never adopts SQL into graph authority. | Gateway and Projection.                  |
| Boundary drift: dbt config lived in passive plugin panels.                                                        | `ConfigureCanvasDbtNode` keeps supported dbt export metadata in route authoring while refusing generated-SQL writes and keeping model-definition policy plugin-owned.                   | DTO plus Command seam.                   |
| Boundary drift: Source could own Transform operations or DVT config could be absent until preview failed.         | `ConfigureCanvasDvtNode` keeps Source identity stable, admits operations only on Transform, and configures writable DVT metadata before projection.                                     | DTO plus Command seam.                   |
...
flowchart TD
  ...
  ConfigureDbt["ConfigureCanvasDbtNode"]
  DbtArtifact["projectDbtModelArtifact"]
  ColumnLineage["projectCanvasColumnLineage"]
  ...
  Canvas --> ConfigureDbt
  ConfigureDbt --> DbtArtifact
  DbtArtifact --> ColumnLineage
  ColumnLineage --> Canvas
  NodeMenu --> Canvas
  CreateNode --> Canvas
  RemoveEdge --> Canvas
  Canvas --> CanvasList
...
- Do not persist derived DVT column-lineage edges as another transform
  authority; `ConfigureCanvasDvtNode` owns the canonical Substrait document and
  later queries project edges from it.
- Do not persist derived dbt column-lineage edges as graph edges;
  `ConfigureCanvasDbtNode` owns the generated-model output selection and
  `projectDbtModelArtifact` plus `projectCanvasColumnLineage` project it.
