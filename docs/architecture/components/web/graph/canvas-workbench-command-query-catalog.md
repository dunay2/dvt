...
| `RemoveCanvasEdgeFromContext`              | Remove the editable edge selected by an edge context-menu gesture through its existing semantic or graph edge-change lifecycle.                                                              | edge id and projected removal capability from `CanvasContextMenuTarget`                                                   | semantic column command or `CanvasEdgeRemovalChange` consumed by `canvasGraphLifecycle.edge` | Canvas edge lifecycle command seam        | `canvasInteractionCommandSurface.ts`, `CanvasViewport.tsx`, `useCanvasControllerReadModel.ts`, `useCanvasEdgeChangeHandlers.ts`                                   | graph edit permission plus the edge's projected removal capability                                                           | read-only posture and non-editable derived lineage expose no delete action; unknown graph edge ids become lifecycle no-op through the existing React Flow edge-change path                                                                                                             |
