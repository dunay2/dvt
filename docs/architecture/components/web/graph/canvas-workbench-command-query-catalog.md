...
| `ListProjectWorkspaceResources`       | query   | proposed      | Project workspace explorer    | `ProjectWorkspaceResourceCatalog`       |
| `AttachProjectResourceToCanvasObject` | command | proposed      | Canvas workbench authoring    | `CanvasResourceAttachmentPolicy`        |
...
| `ListProjectWorkspaceResources`       | Return existing project resources for the Canvas Workspace Explorer.    | project/workspace context, imported resource projection, connector capability summary      | grouped `ProjectWorkspaceResourceCatalog` rows           | Workspace resource query port           | `DbtExplorer`, future `canvasWorkspaceExplorerModel`                        | project scoped; read-only; connector-scoped capabilities            | missing connector, unauthorized resource group, stale import, and unsupported resource kind cannot fabricate rows                      |
| `AttachProjectResourceToCanvasObject` | Attach an existing project resource to a graph object or selected card. | selected canvas object, resource reference, compatibility policy, draft revision           | accepted attachment command or typed rejection           | Canvas draft command port               | future explorer drag/drop adapter, Inspector drop target, graph drop target | graph edit permission plus compatible target kind                   | read-only draft, incompatible resource, stale draft, and missing resource reject                                                       |
...
| Boundary drift: existing resources can be dragged into graph/cards.       | `AttachProjectResourceToCanvasObject` names the compatibility command before broader drop behavior expands.    | Policy Object plus Command seam.     |
