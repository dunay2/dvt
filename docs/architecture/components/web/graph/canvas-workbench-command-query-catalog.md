...
| `ConfigureCanvasDbtNode`              | Edit dbt card package, source, table, materialization, ordered generated-model output columns, and plugin-owned model definition projection metadata.                        | selected `CanonicalNode`, `DbtNodeAuthoringMetadata`, connected-origin column catalog                                     | updated Canvas draft node override                                                           | Inspector and card authoring command seam | `CanvasInspectorAuthoringSection`, `DbtAuthoringFields`, `canvasDbtAuthoringModel`, `projectDbtModelArtifact`, graph-node column controls                   | current Canvas draft and active runtime policy                                                                               | blank required fields, unsupported materialization, unknown or duplicate selected columns, and read-only posture reject; core Canvas cannot invent plugin-specific model-definition rules                                                                         |
