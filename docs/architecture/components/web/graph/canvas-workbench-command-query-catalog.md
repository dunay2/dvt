...
| `ResolveCanvasContextMenu`                 | Return the app-owned contextual action read model for a Canvas background or edge gesture.                                           | `CanvasContextMenuTarget`, active authoring node catalog, effective graph mutation posture            | `CanvasContextMenuModel`                                                              | Canvas interaction query seam           | `canvasInteractionCommandSurface.ts`, `CanvasViewport.tsx`                                    | route-local presentation only; mutation actions are absent unless graph mutation is allowed                                  | read-only posture returns no mutating actions; pane target returns node creation only; edge target returns edge actions only                                                                   |
...
| Boundary drift: browser context menus and edge gestures could own graph intent.                             | `ResolveCanvasContextMenu`, `CreateCanvasAuthoringNode`, and `RemoveCanvasEdgeFromContext` keep contextual actions behind named seams. | Presentation Model plus Command Gateway. |
