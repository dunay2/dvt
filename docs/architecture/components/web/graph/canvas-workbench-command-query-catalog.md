...
| Boundary drift: dbt config lived in passive plugin panels.                                                        | `ConfigureCanvasDbtNode` keeps supported dbt card state in route authoring while refusing generated-SQL writes and keeping model-definition policy plugin-owned.                        | DTO plus Command seam.                   |
