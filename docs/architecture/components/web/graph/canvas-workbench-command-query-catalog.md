...
| `ImportProjectSnapshot`              | Import a validated snapshot into the workspace draft.             | validated `ProjectSnapshot`, draft revision, idempotency key                               | protected draft save receipt or conflict posture         | Canvas draft lifecycle command port     | `useCanvasDraftLifecycle(...)`, `CanvasDraftRepository.saveGraphDraft(...)` | current workspace scope and existing `SaveWorkspaceGraphDraft` auth | invalid file does not save; stale revision conflicts; read-only draft rejects through existing save rail                               |
