...
| `ConfigureCanvasDvtNode`                   | Edit DVT source binding, canonical Substrait field projections and calculated outputs, exclusive SQL or visual-recipe transform authority, and sink materialization metadata before preview. | selected `CanonicalNode`, `DvtNodeAuthoringMetadata`, optional typed Substrait projection or `VisualTransformRecipeV1`    | updated Canvas draft node override                                                           | Inspector and card authoring command seam | `CanvasInspectorAuthoringSection`, `DvtAuthoringFields`, `canvasDvtAuthoringModel`, `canvasDvtTransformAuthoringAuthority`, graph-node column controls            | current Canvas draft and active runtime policy                                                                               | blank or duplicate calculated-field aliases, unsupported expressions or plan shapes, invalid or dual transform authority, unsupported sink materialization or write mode, and read-only posture reject                                                                                 |
