...
| `GenerateTransformationWorkspaceArtifacts` | Render canonical Substrait authoring or read an existing workspace artifact, then persist transformation preview provenance.                                                 | scoped DVT nodes and edges, active workspace scope, optional Git provenance config                                        | saved generated SQL artifact when authoring, saved graph artifact, and preview provenance refs | Workspace file-content command port       | `canvasPreviewProvenance`, governed Substrait renderer, `previewDesignGraphArtifact`, `canvasPlanAction`                                                          | current workspace file write authorization; local draft provenance only for canonical authoring nodes without workspace paths | uninitialized authoring transforms reject; file-backed paths require resolvable files; render or write failure blocks preview; generated paths remain deterministic                                                                                                                    |
...
| Hidden authority: canonical DVT authoring could look executable before a governed render existed.                 | `GenerateTransformationWorkspaceArtifacts` renders the canonical semantic document into deterministic preview artifacts before Plan.                                               | Projection plus Gateway.                 |
