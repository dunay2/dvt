...
- shared-kernel contract closure
...
| FD-DEC-03 | Standalone shared-kernel contracts             | `SelectionContext`, `WorkspaceTab`, and `WorkspaceLayout` are too important to remain implied only in larger docs. | Publish dedicated canonical spec pages for those three contracts and link them from the DDD target doc.                                                                   |
...
1. close FD-DEC-03 by publishing shared-kernel contract docs
2. close FD-DEC-04 by publishing the frontend ACL map
