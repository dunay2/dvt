...
| Component engineering records   | `planning_query_store.governance_component_engineering_record_query` plus component, file, remediation, task, and test joins              | Git edit to governed sources, then `pnpm governance:refresh` and `pnpm planning:db:migrate`                | `pnpm planning:db:query cer --component <component_id>`                                                                      | JSON component engineering record with explicit completeness gaps                      | `pnpm test:planning:db`, `pnpm planning:db:query cer --component <component_id>`                       | DB-first          |
