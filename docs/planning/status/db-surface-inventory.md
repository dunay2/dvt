...
| Component engineering records   | Relational read views `component_engineering_component_query`, `component_engineering_document_query`, `component_engineering_file_query`, `component_engineering_file_rollup_query`, `component_engineering_relation_query`, `component_engineering_contract_query`, and `component_engineering_gap_query`; JSON CER is a final render projection | Git edit to governed sources, then `pnpm governance:refresh` and `pnpm planning:db:migrate`                | `pnpm planning:db:query cer --component <component_id>` plus direct DB reads of normalized component-engineering views       | JSON component engineering record with explicit completeness gaps                      | `pnpm test:planning:db`, `pnpm planning:db:query cer --component <component_id> --schema-version v2`   | DB-first          |
