...
| `config.ts`, `engine-types.ts`                                                                                                                                          | `INFRA`/DTO                                             | `N/A`        | `OK`   | `IPlanFetcher` remains engine-owned artifact fetch vocabulary, but Temporal runtime composition no longer exposes it as dispatch authority. |
...
- **Closed local S08 drift**: workflow activities now resolve execution
  ...
  `ResolvedRunContext`; `IPlanFetcher` is no longer the activity dependency or
  worker runtime authority.
- **Residual S08 drift outside this package**: engine fetch and Postgres/API
