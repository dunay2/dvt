...
| `IRunStateStoreWrite.reserveRetryAttempt(tenantId, sourceRunId)`    | `PORT` (run aggregate identity allocator)      | `CMD-RET`    | `OK`               | Idempotent reservation primitive.                                                                                                                                                                 |
...
| `IPlanResolver.fetch(planRef): ResolvedPlan`                                       | `PORT`                                 | `FETCH`           | **`LEGACY-DRIFT`** | Same scope drift as `IPlanFetcher`. Should require scope.                                          |
| `IPlanArtifactReader.IPlanFetcher.fetch(planRef)`                                  | `PORT` (engine-owned executable fetch) | `FETCH`           | **`LEGACY-DRIFT`** | S08-DRIFT-06.                                                                                      |
| `IPlanArtifactReader.IPlanIntegrityValidator.fetchAndValidate(planRef, fetcher)`   | `DS`                                   | `FETCH`           | **`LEGACY-DRIFT`** | Same scope drift.                                                                                  |
...
| Symbol                                                                       | DDD   | C&Q   | Legacy                                                                                                 | Notes |
| ---------------------------------------------------------------------------- | ----- | ----- | ------------------------------------------------------------------------------------------------------ | ----- |
| Re-exports of ports, services, in-memory adapters, security, core, contracts | `N/A` | `N/A` | `OK` (with the `LEGACY-DRIFT` flags above on `IPlanFetcher`/`IPlanResolver`/`IPlanIntegrityValidator`) |       |
...
### 5.1 Plan-store ports — `src/ports/{IPlanStoreReader,IPlanStoreWriter}.ts`

| Symbol                                                        | DDD                                    | C&Q   | Legacy             | Notes                                                                      |
| ------------------------------------------------------------- | -------------------------------------- | ----- | ------------------ | -------------------------------------------------------------------------- |
| `IPlanStoreReader.getPlanRecord(planId)`                      | `PORT` (read model)                    | `QRY` | **`LEGACY-DRIFT`** | S08 PS-Q01: must become `getPlanRecord(scope, planId)`.                    |
| `IPlanStoreReader.getPlanRecordByRef(planRef)`                | `PORT`                                 | `QRY` | **`LEGACY-DRIFT`** | S08 PS-Q02.                                                                |
| `IPlanStoreReader.listExecutabilityByAdapter(planId)`         | `PORT`                                 | `QRY` | **`LEGACY-DRIFT`** | S08 PS-Q03.                                                                |
| `IPlanStoreReader.getAdmissionLinks(planId)`                  | `PORT`                                 | `QRY` | **`LEGACY-DRIFT`** | S08 PS-Q04.                                                                |
| `IPlanStoreReader.getSupersession(planId)`                    | `PORT`                                 | `QRY` | **`LEGACY-DRIFT`** | S08 PS-Q05.                                                                |
| `IPlanStoreWriter.createPlanRecord(record)`                   | `PORT` (PlanRecord aggregate creation) | `CMD` | **`LEGACY-DRIFT`** | S08 PS-C02; record lacks scope tuple.                                      |
| `IPlanStoreWriter.recordExecutability(record)`                | `PORT`                                 | `CMD` | **`LEGACY-DRIFT`** | S08 PS-C03.                                                                |
| `IPlanStoreWriter.markAdmitted(link)`                         | `PORT`                                 | `CMD` | **`LEGACY-DRIFT`** | S08 PS-C04. Underused — admission auth not bound to it yet (S08-DRIFT-05). |
| `IPlanStoreWriter.markSuperseded(planId, supersededByPlanId)` | `PORT`                                 | `CMD` | **`LEGACY-DRIFT`** | S08 PS-C05.                                                                |
| `IPlanStoreWriter.archivePlan(planId, archivedAtIso)`         | `PORT`                                 | `CMD` | **`LEGACY-DRIFT`** | S08 PS-C06.                                                                |
...
- Compiled-code, dbt-bundle, and run-execution-context surfaces are well-shaped.
- **Active drift**: every method on `IPlanStoreReader`/`IPlanStoreWriter` is
  unscoped (S08 matrix). This is the largest single block of legacy in the
  package and gates the S08 closure.
- Minor `computeSha256` duplication.
...
| `IPlanExecutabilityValidator.validatePlan(planRef, adapterId)`                            | `PORT` (executability validator) | `QRY`                 | **`LEGACY-DRIFT`** | S08-DRIFT-31. Unscoped — must require `(scope, planRef, adapterId)`. |
| `IPlanValidationLifecycleStore.storePlan(buildResult)`                                    | `PORT` (lifecycle store)         | `CMD-RET`             | **`LEGACY`**       | S08 PS-C01 (legacy facade). Replace with scoped command and remove.  |
| `IPlanValidationLifecycleStore.markValid(planRef)`                                        | `PORT`                           | `CMD`                 | **`LEGACY`**       | S08 PS-C07.                                                          |
| `IPlanValidationLifecycleStore.markInvalid(planRef, report)`                              | `PORT`                           | `CMD`                 | **`LEGACY`**       | S08 PS-C08.                                                          |
| `IPlanValidationLifecycleStore.getValidationRecord(planId)`                               | `PORT`                           | `QRY`                 | **`LEGACY`**       | S08 PS-Q06.                                                          |
...
| Symbol                                                             | DDD | C&Q | Legacy             | Notes                                                       |
| ------------------------------------------------------------------ | --- | --- | ------------------ | ----------------------------------------------------------- |
| Re-export of `IPlanValidationLifecycleStore`                       | n/a | n/a | **`LEGACY`**       | S08-DRIFT-12. Public lifecycle import path must be removed. |
| Re-export of `IPlanExecutabilityValidator` (unscoped)              | n/a | n/a | **`LEGACY-DRIFT`** | S08-DRIFT-31.                                               |
...
| ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------- | ----------------- | ------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `PostgresRunStateStoreAdapter` (impl `IRunStateStore`)                                                                                                                                | `ADP`                                       | `CMD/QRY`         | `OK`   | All methods tenant-scoped: `bootstrapRunTx`, `appendAndEnqueueTx`, `getRunMetadataByRunId`, `listRuns`, `saveProviderRef`, `reserveRetryAttempt`, `listEvents`, `getSnapshot`, `pinTerminalSnapshot`, `getPinnedTerminalSnapshot`, `rebuildSnapshot`. |
...
| `PostgresRelationalExecutionCapability` / `PostgresServiceAccessCapability` | `DS` (capability)                    | `QRY`                              | `OK`                                                                |                                                    |
...
self-hosted Temporal (ADR-0014). 52 source files. The package contains the
...
| `config.ts`, `engine-types.ts`                                                                                                                                          | `INFRA`/DTO                                             | `N/A`        | `OK` (S08-DRIFT-26 risk: `engine-types` propagates unscoped `IPlanFetcher`) |                                                                                                                          |
...
| `activityTypes.ts`          | `N/A` (DTO)                   | `N/A`             | `OK` (S08-DRIFT-26 risk: imports unscoped engine `IPlanFetcher`) |       |
...
- Provider adapter, workflow, activities, and plugin layers are well-shaped
  ...
  domain helpers).
- **Drift**: workflow activities and `engine-types` propagate unscoped
  `IPlanFetcher` (S08-DRIFT-15/26). The cure is upstream (engine port shape),
  not in this package.
...
| `PreviewPlanUseCase`                                                                                                                                                                                                                                                                                                                                   | `AS` (preview orchestration)              | `CMD-RET`               | **`LEGACY-DRIFT`**          | S08-DRIFT-04: still uses `IPlanValidationLifecycleStore.storePlan` and `markValid/markInvalid` as authority (S08 PS-C01, PS-C07, PS-C08). |
| `PlannerBackedStartRunUseCase`                                                                                                                                                                                                                                                                                                                         | `AS`                                      | `CMD-RET`               | **`LEGACY-DRIFT`**          | Same lifecycle facade dependency; does not call `markAdmitted` (S08-DRIFT-05).                                                            |
| `BackpressureAwareStartRunUseCase`                                                                                                                                                                                                                                                                                                                     | `AS` (decorator)                          | `CMD-RET`               | `OK`                        | Wraps with backpressure gate.                                                                                                             |
| `engineStartRunUseCase` (with `startRunEngineBridge`, `startRunRouteCommandBuilder`)                                                                                                                                                                                                                                                                   | `AS`                                      | `CMD-RET`               | **`LEGACY-DRIFT`**          | S08-DRIFT-38: direct `PlanRef`→engine handoff without scoped plan-store ownership assertion.                                              |
| `recoverRunUseCase`                                                                                                                                                                                                                                                                                                                                    | `AS`                                      | `CMD-RET`               | **`LEGACY-DRIFT`**          | Same scope drift on recovery.                                                                                                             |
| `cancelRunUseCase`                                                                                                                                                                                                                                                                                                                                     | `AS`                                      | `CMD`                   | `OK`                        |                                                                                                                                           |
| `signalRunUseCase`                                                                                                                                                                                                                                                                                                                                     | `AS`                                      | `CMD`                   | `OK`                        |                                                                                                                                           |
| `getRunStatusUseCase`                                                                                                                                                                                                                                                                                                                                  | `AS` (read model)                         | `QRY`                   | **`LEGACY-DRIFT`**          | S08-DRIFT-33: enriches by `getPlanRecord(planId)` without scope.                                                                          |
| `getRunEventsUseCase`                                                                                                                                                                                                                                                                                                                                  | `AS`                                      | `QRY`                   | `OK`                        | Tenant-scoped via context.                                                                                                                |
| `listRunsUseCase`                                                                                                                                                                                                                                                                                                                                      | `AS`                                      | `QRY`                   | `OK`                        |                                                                                                                                           |
| `ImportPlanUseCase`                                                                                                                                                                                                                                                                                                                                    | `AS`                                      | `CMD-RET`               | **`LEGACY-DRIFT`**          | S08-DRIFT-22: resolves stored plans through unscoped `fetch(planRef)`.                                                                    |
| `StoredExecutablePlanResolver`                                                                                                                                                                                                                                                                                                                         | `DS` (resolver)                           | `QRY` (FETCH+integrity) | **`LEGACY-DRIFT`**          | Same.                                                                                                                                     |
| `StoredPlanExecutabilityValidator`                                                                                                                                                                                                                                                                                                                     | `DS` (validator)                          | `QRY` (assertion)       | **`LEGACY-DRIFT`**          | Unscoped `fetchForValidation(planRef)` — S08-DRIFT-31.                                                                                    |
| `WorkflowEngineFactory`                                                                                                                                                                                                                                                                                                                                | `INFRA` (factory)                         | `QRY`                   | **`LEGACY-DRIFT`**          | S08-DRIFT-22: accepts engine `IPlanFetcher` without scope.                                                                                |
| `notImplementedStartRunUseCase`                                                                                                                                                                                                                                                                                                                        | `INFRA` (placeholder)                     | `CMD`                   | **`LEGACY`**                | Marked not-implemented; should be removed once production composition is final.                                                           |
...
| File                                                                               | DDD    | C&Q           | Legacy             | Notes                                |
| ---------------------------------------------------------------------------------- | ------ | ------------- | ------------------ | ------------------------------------ |
| `storedPlan.ts` (`IStoredPlanValidationReader.fetchForValidation(planRef)`)        | `PORT` | `QRY` (FETCH) | **`LEGACY-DRIFT`** | S08-DRIFT-10/31. Must require scope. |
| Other application ports (admission telemetry, duplicate run probe, capacity, etc.) | `PORT` | mixed         | `OK`               |                                      |

### 13.3 HTTP entrypoints — `src/entrypoints/http/`

| File group                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | DDD                                     | C&Q               | Legacy                                        | Notes                                                               |
| ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------- | ----------------- | --------------------------------------------- | ------------------------------------------------------------------- |
| `compilePlanRoute`, `previewPlanRoute(*)`, `importPlanRoute(*)`, `executePlanRouteFacade`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | `ENTRY` (HTTP route → AS)               | `CMD-RET`         | **`LEGACY-DRIFT` (preview/import only)**      | Drives lifecycle-facade UseCases.                                   |
| `startRunRoute(*)`, `recoverRunRoute(*)`, `cancelRunRoute(*)`, `signalRunRoute(*)`, `runCommandRouteExecutor`, `runCommandFieldParsers`, `runCommandRoute.constants`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                | `ENTRY`                                 | `CMD`             | **`LEGACY-DRIFT`** (start/recover)            | S08-DRIFT-38.                                                       |
| `getRunRoute(*)`, `getRunEventsRoute(*)`, `listRunsRoute(*)`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | `ENTRY`                                 | `QRY`             | `OK` (status enrichment carries S08-DRIFT-33) |                                                                     |
...
| `modules/buildProtectedRuntimeModule.ts`                         | `INFRA` (composition)           | `QRY`         | **`LEGACY-DRIFT`** | S08-DRIFT-22.                                                                          |
| `modules/protectedRuntime/buildProtectedRuntimeStorage.ts`       | `INFRA`                         | `QRY`         | **`LEGACY-DRIFT`** | Same.                                                                                  |
...
| `modules/startRun/buildProtectedStartRunRuntime.ts`              | `INFRA`                         | `QRY`         | **`LEGACY-DRIFT`** | S08-DRIFT-22 (requires `IPlanValidationLifecycleStore & IStoredPlanValidationReader`). |
| `modules/stateStoreRoles.ts`                                     | `DS` (role policy)              | `QRY`         | `OK`               |                                                                                        |
| `modules/types.ts`                                               | `N/A` (DTO)                     | `N/A`         | **`LEGACY-DRIFT`** | Exposes lifecycle plan store and unscoped executable plan resolver to module graph.    |
...
| `runtime/runtimeTypes.ts`, `temporalWorkerRuntimeHandle.ts`, `temporalWorkerRuntimeResources.ts` | `INFRA` (runtime types/lifecycle) | mixed     | **`LEGACY-DRIFT`** | S08-DRIFT-26: carries `PlanFetcherLike`/plan-store resources without tenant-owned scope. |
| `runtime/temporalWorkerStores.ts`                                                                | `INFRA`                           | `QRY`     | **`LEGACY-DRIFT`** | S08-DRIFT-26: wires `PostgresPlanStore` as engine fetcher unscoped.                      |
...
**Verdict**: clean composition aside from S08-DRIFT-26 propagation.
...
### 18.1 Plan-store ports / engine fetcher (S08-DRIFT-06/10/22/26/38/39/40)

**Current (drift)**: `IPlanFetcher.fetch(planRef)`, `IPlanStoreReader.*(planId)`,
`IPlanStoreWriter.*(record-without-scope)` — all globally keyed, with
`PlanRefPolicy` and admission guards mistaken for ownership enforcement.

```mermaid
flowchart LR
  subgraph CurrentDrift[Current — drift]
    A1[HTTP startRun route] -->|planRef + ctx| A2[StartRunUseCase]
    A2 -->|planRef| A3[StoredExecutablePlanResolver]
    A3 -->|fetch planRef| A4[IPlanFetcher]
    A4 -->|SQL by plan_id only| A5[(stored_plans)]
    A6[PlanRefPolicy] -.URI allowlist.- A2
    A7[StartRunAdmissionGuard] -.tenant access.- A2
    note["No row-ownership check\nenforces (tenantId,projectId,environmentId)"]
    A2 -.- note
  end
```

**Target (DDD-shaped, scoped)**:

```mermaid
flowchart LR
  subgraph Target[Target — scoped C&Q]
    B1[HTTP startRun route] -->|StartRunCommand with scope| B2[StartRunApplicationService]
    B2 -->|scope, planRef| B3[ScopedPlanStoreReader<br/>PS-Q08]
...
ScopedPlanStoreReader.fetch(scope, planRef)   <- PS-Q08 [scoped]
   |
   v
...
### 18.2 Lifecycle facade → scoped C&Q (S08-DRIFT-04/09/12)

**Current**: `IPlanValidationLifecycleStore.{storePlan, markValid, markInvalid, getValidationRecord}`
on a single object, mixing aggregate persistence + lifecycle state machine +
audit query — non-DDD (one interface masquerading as four owners).

```mermaid
flowchart TB
  subgraph Legacy[Legacy lifecycle facade]
    L1[IPlanValidationLifecycleStore] --> L2[storePlan]
    L1 --> L3[markValid]
    L1 --> L4[markInvalid]
    L1 --> L5[getValidationRecord]
  end
```
...
flowchart TB
  ...
  Legacy -.removed.-> Aggregate
```

### 18.3 `PostgresPlanStore` repository decomposition (S08-DRIFT-04)

**Current**: a single `PostgresPlanStore` class implementing five different
ports (`IPlanStoreReader & IPlanStoreWriter & IPlanValidationLifecycleStore &
IStoredPlanValidationReader & IPlanFetcher`) with five sibling
`PostgresPlanStore.*-repository.ts` helpers as free-floating SQL — no DDD owner.
...
ENGINE PORT (owned by engine)       ScopedPostgresPlanFetcher
IPlanFetcher.fetch(scope, planRef)  (tenant pred + integrity check)
...
The composer (`PostgresPlanStoreComposer`) becomes the single composition
point; it never re-exposes the legacy facade.
...
- `IPlanValidationLifecycleStore` — single interface with four ownership
  responsibilities. Replace with aggregate + domain service + AS + read model
  (diagram §18.2).
- `PostgresPlanStore` (composite class implementing five ports) — replace with
  one adapter per port + composer (diagram §18.3).
...
- `apps/api.modules/types.ts` — exposes lifecycle plan store as a "type" to the
  module graph; not a DDD concept. Replace with scoped role types.
