...
- [Task: GOV-PROP-DISP-1] add rollback instructions in case docs discoverability regresses
...
- [Task: GOV-PROP-DISP-1] add a post-migration runbook for docs contributors and release managers
- [Task: GOV-PROP-DISP-1] add a CI smoke assertion that docs build output contains extracted planning
  pages
- [Task: GOV-PROP-DISP-1] add a short rollback playbook for temporary re-tracking in incident mode
...
- [Task: GOV-PROP-DISP-1] `CDG-W4-2` from the CI Delivery Governance Consolidated Action Plan
- the broader Autogenerated Pages Extraction Plan

It should land before any repo-wide extraction of generated docs families.

## Relationship To GOV-S3

This plan is still limited to planning-generated surfaces: lane Markdown,
workboard, open-task-route, and planning landing indexes.

The broader storage proposal for moving both planning state and
`system-governance-*` read models behind a derived Postgres query store is
[Planning And Governance Query Store Plan](./planning-state-query-store-plan-20260506.md).

Use that GOV-S3 plan for:

- `system-governance-file-index.*`
- `system-governance-component-index.*`
- `system-governance-component-file-map.*`
- `system-governance-file-fingerprint-*`
- `system-governance-coverage-report.*`
- `system-governance-remediation-queue*`
- `governance-files/**`
- `governance-components/**`

This file only carries a narrow 2026-05-06 mechanization manifest because the
lane-state reality reconciliation touched Lane A, Lane B, Lane D, Lane E, and
the generated governance status files affected by those lane-source changes.
It does not own the GOV-S3 query-store architecture.

## 2026-05-06 Lane-State Reality Reconciliation Mechanization

This manifest authorizes only the cross-lane planning registry reconciliation
that updates Lane A, Lane B, Lane D, Lane E, and the generated governance status
fingerprints affected by those lane-source changes. It does not authorize
planning query store, GOV-S3, product code, adapter, contract, CI, or workflow
changes.

```feature-mechanization
version: 1
featureId: PLANNING-LANE-STATE-REALITY-RECONCILIATION-20260506
...
forbiddenImplementationSurfaces:
  ...
  - scripts/**
  - tools/**
  - docs/planning/proposals/mandatory/governance-and-docs/*planning-query-store*
  - docs/planning/proposals/mandatory/governance-and-docs/*GOV-S3*
commandQueryRails:
  - name: ReviewPlanningLaneRealityState
    type: query
    dddOwner: PlanningLaneRegistry
  - name: AcceptPlanningLaneRealityReconciliation
    type: command
    dddOwner: PlanningLaneRegistry
domainObjects:
  - name: PlanningLaneRegistry
...
architectureGuards:
  ...
  - pnpm docs:feature-mechanization --feature PLANNING-LANE-STATE-REALITY-RECONCILIATION-20260506
...
completionGate:
  ...
  - pnpm verify:prepush
redGreenCycles:
  - id: lane-state-reality-reconciliation-scope
    redTest: pnpm docs:feature-mechanization:implementation
    expectedFailure: Lane B and Lane D YAML changes are outside allowedImplementationSurfaces before this manifest declares the cross-lane lane-state reconciliation surfaces.
    patchSurfaces:
      - docs/planning/proposals/mandatory/governance-and-docs/generated-planning-surfaces-extraction-plan-20260403.md
      - docs/planning/state/agent-lane-a.yaml
      - docs/planning/state/agent-lane-b.yaml
...
symbols:
  - name: PlanningLaneStateRealityReconciliationManifest
    path: docs/planning/proposals/mandatory/governance-and-docs/generated-planning-surfaces-extraction-plan-20260403.md
    dddOwner: PlanningLaneRegistry
    cqRails:
...
- [Task: GOV-PROP-DISP-1] [How to Add Tasks to an Agent Lane](../../../state/how-to-add-tasks.md)
