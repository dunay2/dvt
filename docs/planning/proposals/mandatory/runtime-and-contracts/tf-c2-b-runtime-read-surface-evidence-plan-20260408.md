...
4. failure attribution to canonical step id (`execution.failure.stepId`) with failure reason/message
...
- This development slice intentionally replaces the flat read-surface evidence
  fields with a nested `execution` object instead of preserving compatibility
  with an unstable pre-release DTO.
