...
The rail names below are the canonical command/query names. Historical
`Api*Command` and `Api*Query` labels are retired aliases; API remains the
transport surface, not the product rail vocabulary.

| Rail                        | Type    | Owning DDD object                   | Transport surface                        | App surface                                                         | Notes                                                                 |
| --------------------------- | ------- | ----------------------------------- | ---------------------------------------- | ------------------------------------------------------------------- | --------------------------------------------------------------------- |
| `StartRun`                  | Command | Start run application service       | `POST /runs` via `startRunRoute`         | `BackpressureAwareStartRunUseCase`, `EngineStartRunUseCase`, facade | Retires `ApiStartRunCommand`; blocked by S08 scoped ownership drift   |
| `CancelRun`                 | Command | Run command application service     | cancel route                             | `CancelRunUseCase`                                                  | Retires `ApiCancelRunCommand`; separate from signal compatibility     |
| `SignalRun`                 | Command | Run command application service     | signal route                             | `SignalRunUseCase`                                                  | Retires `ApiSignalRunCommand`; negative tests own unsupported signals |
| `RecoverRun`                | Command | Run recovery application service    | recover route                            | `RecoverRunUseCase`                                                 | Retires `ApiRecoverRunCommand`; must not bypass tenant scope          |
| `CompilePlan`               | Command | Plan compile application service    | compile route                            | `CompilePlanUseCase`                                                | Retires `ApiCompilePlanCommand`; build artifact result command        |
| `PreviewPlan`               | Command | Plan preview application service    | preview route                            | `PreviewPlanUseCase`                                                | Retires `ApiPreviewPlanCommand`; drift until lifecycle facade removal |
| `ImportPlan`                | Command | Plan import application service     | import route                             | `ImportPlanUseCase`                                                 | Retires `ApiImportPlanCommand`; drift until scoped stored-plan fetch  |
| `SaveWorkspaceGraphDraft`   | Command | Workspace draft application service | workspace draft route                    | `SaveWorkspaceGraphDraftUseCase`                                    | Retires `ApiSaveWorkspaceGraphDraftCommand`; capability-gated         |
| `GetWorkspaceGraphDraft`    | Query   | Workspace draft read model          | workspace draft route                    | `GetWorkspaceGraphDraftUseCase`                                     | Retires `ApiGetWorkspaceGraphDraftQuery`; tenant/project scoped       |
| `GetRunStatus`              | Query   | Run status read model               | get run route                            | `GetRunStatusUseCase`                                               | Retires `ApiGetRunStatusQuery`; plan enrichment drift remains         |
| `GetRunEvents`              | Query   | Run event read model                | events route                             | `GetRunEventsUseCase`                                               | Retires `ApiGetRunEventsQuery`; cursor and tenant guard required      |
| `ListRuns`                  | Query   | Runs read model                     | list route                               | `ListRunsUseCase`                                                   | Retires `ApiListRunsQuery`; tenant guard required                     |
