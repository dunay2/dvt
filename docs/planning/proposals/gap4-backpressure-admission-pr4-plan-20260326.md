---
title: G4-PR4 Operability & Metrics — Implementation Plan
...
admission outcome — accept, duplicate, reject_tenant, reject_system, would_reject_tenant,
...
2. Capacity gauges (queue depth, outbox lag) are not emitted — `RawSqlBackpressureStore`
...
┌─────────────────────────────── Application Layer ───────────────────────┐
│                                                                           │
│  PORT: AdmissionTelemetry             PORT: IBackpressureCapacityTelemetry│
│  record(AdmissionDecisionRecord)      recordSnapshot(input)               │
│                                                                           │
│  BackpressureAwareStartRunUseCase                                         │
│    ├── deps.telemetry: AdmissionTelemetry          (decision events)      │
│    └── deps.admissionGuard: AdmissionGuard                                │
│                                                                           │
└─────────────────────────────────────────────────────────────────────────┘
             ↑ implements                   ↑ implements
┌─────────────────────────────── Infrastructure Layer ───────────────────┐
│                                                                          │
│  ObservabilityAdmissionTelemetry      ObservabilityBackpressureCapacity  │
│  (NEW — replaces Noop)                Telemetry (NEW)                    │
│                    ↓ uses                         ↓ uses                 │
│                    IObservability                 IObservability          │
│                                                                          │
│  MetricsEmittingBackpressureStore  (NEW — Decorator pattern)             │
│    wraps: BackpressureSnapshotEnvelopeStore                              │
│    exposes: BackpressureStore                                            │
│    calls: IBackpressureCapacityTelemetry.recordSnapshot() on each read  │
│                                                                          │
│  Store chain:                                                            │
│    RawSqlBackpressureStore                                               │
│      → CircuitBreakingBackpressureStore                                  │
│        → CachedBackpressureStore                                         │
│          → MetricsEmittingBackpressureStore  ← NEW position             │
...
| ----------------------------- | ---------------------------------------------------------------------------- | ------------------------------------------------------------------------------------ |
| **S** — Single Responsibility | `AdmissionTelemetry` mixes decision recording with capacity reporting        | Split into two ports                                                                 |
| **O** — Open/Closed           | Decision variants require modifying the bag struct                           | Discriminated union: extend by adding a variant, not modifying existing              |
| **I** — Interface Segregation | One port with 4 optional fields used by different call sites                 | Two ports, each with exactly the fields its callers need                             |
| **D** — Dependency Inversion  | `buildProtectedRuntimeModule` depends on `NoopAdmissionTelemetry` (concrete) | Depend on `AdmissionTelemetry` port; inject the real adapter at the composition root |

---

## Ports

### 1 — Refine `AdmissionTelemetry` (existing port — breaking change on signature)
...
### 2 — New port: `IBackpressureCapacityTelemetry` (ISP split)
...
are swallowed internally. This is not a command — it is an observation.

---

## Infrastructure Adapters (new files)

### 3 — `admissionTelemetryMetrics.ts`
...
- `dvt.admission.decision_total` → `{ mode, decision }`
- `dvt.admission.rejection_total` → `{ mode, decision, code }`
- `dvt.admission.pending_events_per_tenant` → `{ source }` (no `tenantId` — high-cardinality)
- `dvt.admission.outbox_oldest_age_ms` → `{ source }`

### 4 — `ObservabilityAdmissionTelemetry.ts`
...
| --------------------- | --------------------------------- | --------- | ---------------------------------------- |
| `accept`              | `decision_total {mode, decision}` | `info`    | —                                        |
| `duplicate`           | `decision_total {mode, decision}` | `info`    | —                                        |
...
handled here — this is the OCP compliance guarantee.

Log attributes (info/warn only — no high-cardinality in metric labels):
...
### 5 — `ObservabilityBackpressureCapacityTelemetry.ts`
...
### 6 — `MetricsEmittingBackpressureStore.ts`
...
### 7 — Update `BackpressureAwareStartRunUseCase`

`recordDecision` → `record`, all call sites updated to pass the typed variant.
No logic change — only the call signature adapts to the new port.

### 8 — Update `NoopAdmissionTelemetry`
...
### 9 — Update `buildProtectedRuntimeModule.ts`
...
Tests are written before implementation. Each test maps to a red → green → refactor cycle.
...
| 12  | `swallows internal errors — must not throw`                           | observability spy throws → `record()` resolves without throwing                       |
...
| 5   | `swallows telemetry errors — snapshot still returned`       | telemetry spy throws → snapshot resolves |
| 6   | `propagates delegate errors`                                | delegate throws → store throws           |
...
C1  refactor(admission-port): replace optional-field bag with AdmissionDecisionRecord union
    - AdmissionTelemetry.ts: recordDecision → record, input → AdmissionDecisionRecord
...
| File                                                                    | Change                                     |
| ----------------------------------------------------------------------- | ------------------------------------------ |
| `apps/api/src/application/ports/AdmissionTelemetry.ts`                  | Port refined — discriminated union         |
...
| stuck backlog metrics and alerts exist                | C7, C9 | `outbox_oldest_age_ms` gauge — high value = stuck; `pending_events_per_tenant` gauge           |
