...
| CI-OPT-2 | Gate `markdown-lint` on `docs_changed`                               | **Implemented** — `needs: detect-affected`, `if: docs_changed == 'true'`                              |
| CI-OPT-3 | Gate 7 doc-checks on `docs_changed`                                  | **Implemented** — `scope.outputs.docs_changed` gates each step in `pr-checks`                         |
| CI-OPT-4 | Remove duplicate `adapter-postgres-smoke` from `pr-quality-gate.yml` | **Implemented** — job is gone from `pr-quality-gate.yml`                                              |
| CI-OPT-5 | `fetch-depth` conditional in non-PR runs                             | **Implemented** — `${{ github.event_name == 'pull_request' && '0'                                     |     | '1' }}` |

BUG-1 through BUG-C from the prepush observations review: **Implemented** — scripts fixed.
...
## Section 1 — Correctness Issues (Blocking Quality)

### C1 — `test:determinism` and `test:replay` use `|| true` (CRITICAL)
...
These are the most safety-critical tests in the repository. The execution model's correctness guarantee rests on them. A regression in determinism that breaks replay — the kind that would produce split-brain state in Temporal — will not be caught by CI.
...
### C2 — ADR-0000 Traceability Gate Is Opt-In Only
...
**Fix:** Move the traceability gate to run on every `push` to `main` (not every PR — it can be slow). Remove the `workflow_dispatch`-only guard or add a separate always-on check. At minimum, it should block main merges when ADR coverage regresses.

---

### C3 — `any` Type Check in Contracts Is a Regex Grep

```bash
# contracts.yml — contract-compile job
...
**Fix:** Remove the grep. Rely on `tsc --noEmit` with `"strict": true` in `tsconfig.json`. If explicit `any` escape hatches need banning, add `@typescript-eslint/no-explicit-any` to the ESLint rules for the contracts package — this is semantically correct and covers all cases the grep misses.

---

### C4 — `contracts.yml` Golden Validation Skipped on `workflow_dispatch`

```yaml
# contracts.yml — contract-validate job
if: >
...
This condition means: when someone manually triggers `workflow_dispatch` without opting into `run_golden_validation`, neither `contract-validate` nor `contract-hashes` (same pattern) runs. Manual CI re-runs — the common tool for debugging flaky tests — silently skip the most important contract correctness gate.
...
### G1 — `@dvt/planner`, `@dvt/delivery`, and Other Packages Not in Test Scope
...
- `workspace-ci` build + typecheck in `ci.yml` (if planner is in the matrix — but it's NOT, planner is not listed in the detect-affected matrix)
...
### G3 — `workspace-ci` Matrix Does Not Include All Source Packages
...
### G4 — API Integration Tests Not Wired to the Test Matrix
...
This runs `pnpm --filter dvt-api test` — unit tests only. The integration test (`pnpm --filter dvt-api test:integration`) requires a running Postgres. There is no job in `test.yml` or `contracts.yml` that runs API integration tests with a Postgres service.
...
### S1 — `pr-checks` Is a Monolithic 30-Minute Sequential Job

The `pr-checks` job contains 20+ sequential steps:
ARC policy → ARC docs check → docs sync check → docs workboard check → docs quality → docs doctor → Markdown location → canonical docs → Markdown lint → code-state check → capability check → `pnpm type-check` (6 builds + tsc) → PR title → PR size → PR description → labeler → scope detection.
...
- `pr-metadata` — PR title, size, description, labeler. Fast, no install needed.
- `pr-quality` — ARC check, docs gates, type-check.
...
### S2 — Concurrency Group for Push-to-Main in `pr-quality-gate.yml` Uses `run_id`
...
For PR events: `github.event.number` is the PR number → correct cancellation.
For push to main: `github.event.number` is empty → falls back to `github.run_id` → each push spawns a unique group → **no cancellation between concurrent main pushes**.
...
### S3 — Scope Detection Is Duplicated Across Four Workflows
...
Missing any one of them creates a silent coverage gap (as seen in G1 and G3 above — `@dvt/planner` is missing from `ci.yml` but would need to be added to `test.yml` independently).
...
### S4 — No `dependabot.yml` for GitHub Actions
...
### S5 — `lint-staged` Does Not Cover `scripts/*.cjs`
...
### S6 — `pnpm type-check` Rebuilds 6 Packages Sequentially on Every Run
...
In `pr-quality-gate.yml`, this runs inside `pr-checks` which already has the `node_modules` cache warm. But the build outputs (`dist/`) are not cached — every run rebuilds from TypeScript source. For packages that didn't change, this is wasted compute.
...
## Section 4 — Process Observations

### P1 — No Merge Queue Configuration
...
### P2 — `verify:prepush` Does Not Run Tests
...
A developer who pushes code that compiles and lints correctly but breaks 50% of the test suite will see green locally and only find out tests are broken after CI runs (15–20 minutes later).
...
### P3 — QUALITY-1 (2-dot vs 3-dot diff) Still Deferred
...
## Section 5 — Improvement Priority Table
...
### C1 — Fix `test:determinism || true` (implement immediately, 30 minutes)
...
### G1 + G3 — Add `@dvt/planner` to CI scope (implement in one PR)
...
### S4 — Dependabot for GitHub Actions (implement immediately, 10 minutes)

Create `.github/dependabot.yml` with the configuration shown in S4 above. This has zero risk and prevents security drift.

### C2 — ADR-0000 Traceability Gate (implement in a dedicated CI PR)
