...
Lo que es frágil
stepTypeConfig: Record<string, unknown> es un contrato roto disfrazado de extensibilidad. El planner genera { stepTimeoutMs, retries, concurrency, custom } dentro de este campo opaco. El adapter-temporal lee este blob sin validación tipada. No hay un contrato compartido que enforque la forma. G9 existe precisamente porque esto se reconoce como problema, pero llevan con este hueco operando todas las demás piezas. El día que el adapter-temporal no encuentre stepTimeoutMs o encuentre un valor null, falla silenciosamente en runtime porque TypeScript no puede ayudar con Record<string, unknown>.
...
StepTypeRegistry (G9) no existe. La extensibilidad del sistema depende de que alguien escriba stepTypeConfig con la forma correcta y el adapter la lea correctamente. Hasta que G9 esté cerrado, el "plugin system" para step types es inoperante como abstracción verificable.
...
stepTypeConfig sin contrato causa divergencia adapter/planner Alta Alta G9 Pending; el adapter-temporal lee campos sin validación tipada Cerrar G9. StepTypeRegistry con schema Zod por kind.
...
Extensibility 4/10 StepTypeRegistry no existe. Plugin runtime a 4%. stepTypeConfig opaco. StepKind es string. La "extensibilidad" está declarada en docs pero no implementada como mecanismo.
...
Media: 6.1/10. Sistema con fundamentos conceptuales sólidos pero sustancial deuda de implementación y operability.
...
1. Cerrar G9 (StepTypeRegistry) antes de cualquier trabajo de plugin.
   El sistema no puede tener un registry de plugins extensible si el registry de step types no existe. stepTypeConfig: Record<string, unknown> es el mayor hueco de contrato en el codebase. Mientras no exista un IStepTypeConfig<T> tipado y validado por kind, cualquier adapter nuevo leerá el blob y fallará silenciosamente. G9 es prerequisito de G10, del plugin system, y de cualquier adapter de Conductor.
