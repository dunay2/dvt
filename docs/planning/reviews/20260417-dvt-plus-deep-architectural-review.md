...
- **No formal plan/event schema evolution matrix test**. ADR-0036 declares a registry and runtime admission matrix. Contract vector fixtures exist. There is no pairwise test that asserts `{planVersion, runtimeName} ∈ PLAN_RUNTIME_ADMISSION_MATRIX ↔ engine accepts/rejects`. The mechanism exists; the enforcement surface does not.
...
| R12 | Planner and plan-verifier drift on `planVersion` registry       | Medium      | Medium                          | Contracts own the registry; plan-verifier owns the runtime admission matrix. Two sources. A mismatch is silently acceptable in contract tests that do not cross-check.                                                                                                                         | Add a contract test in a neutral workspace that asserts `SUPPORTED_EXECUTION_PLAN_VERSIONS ⊆ PLAN_RUNTIME_ADMISSION_MATRIX.keys()` and every key has at least one runtime verdict.                                                                                                                                                  |
...
| T9  | Add CI check: every `planVersion` in `SUPPORTED_EXECUTION_PLAN_VERSIONS` must exist as a key in `PLAN_RUNTIME_ADMISSION_MATRIX` with at least one runtime verdict.                                                                                                                         | P2       | 1 d    | CI fails on mismatch.                                                                                                           | `pnpm validate:contracts`, new contract test                                |
