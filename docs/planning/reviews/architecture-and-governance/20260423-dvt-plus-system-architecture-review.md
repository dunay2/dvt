...
| Tenant identity                | `runId` is caller-provided from web as `run_ui_${Date.now()}`. Postgres `run_metadata` uses `run_id` as primary key. ADR-0031 says tenant-owned rows must be scoped, but the schema still makes run id globally unique.                                                                             |
...
| Tenant-scoped plan and run identity policy | Either run ids are globally generated by the platform, or the schema must scope them by tenant. Current code mixes caller-owned ids with global uniqueness.          |
...
| Multi-tenant isolation flaw  | High     | Medium     | `run_id` is globally unique while callers provide it; plan records lack indexed tenant columns; some lineage migrations backfill `__unknown_tenant__`.                                       | Decide global platform-owned run ids or composite tenant-scoped keys; add top-level tenant columns to plan records; add continuous cross-tenant tests. |
...
`run_snapshots`. That can be acceptable only if run ids are platform-generated
globally unique identifiers.

The UI currently generates run ids from `Date.now()`. That is not a durable
...
| Tenant identity model         | Decide whether `runId` is globally platform-owned or tenant-scoped. Current UI-generated ids are not enough.            |
...
| P0       | Tenant/run identity decision      | Decide global platform-generated `runId` versus tenant-scoped composite identity; stop UI timestamp ids from being authoritative. | ADR or contract update plus API-side generation/validation tests.                        |
...
The next work should be truth correction, tenant identity, capability
fail-closed behavior, read-side contracts, plugin isolation, and operational
proof. More abstraction would be the wrong move.
