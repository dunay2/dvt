...
| F22 | Medium   | Open      | `PostgresRunEventStore` still clamps `limit` to at least `1`, which can alter caller semantics for explicit `limit=0` reads.                                      | `listEvents` applies `Math.max(1, options.limit)` before querying instead of honoring explicit zero as a valid boundary request.                                                                    |
...
| F24 | Low      | Open      | Minor import duplication/ordering drift in `PostgresRunEventStore` adds noise and weakens maintainability conventions.                                            | Same module path is imported in separate statements instead of single grouped import block.                                                                                                         |
